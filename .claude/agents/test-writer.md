...
> **Stack note**: tests run on Vitest via `@stencil/vitest` (Stencil's official Vitest wrapper). The Jest/`newSpecPage` stack is retired — see `vitest.config.mts` + `vitest-setup.ts`. `yarn test` invokes `vitest run --project spec` directly. It builds nothing: the `spec` project compiles components from source through `stencilVitestPlugin()`, so no `dist/` bundle is involved.
