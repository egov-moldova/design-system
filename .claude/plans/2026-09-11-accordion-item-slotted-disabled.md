...
| 22 | A `tabindex` written by anything else during the disabled window is suppressed again at the next sync | Spec test "re-suppresses a `tabindex` written by anyone else during the disabled window", mutation-confirmed against the previous skip-if-claimed guard. The two mirrors now enforce identically: `disabled` re-asserts because it reads the live attribute, `tabindex` because it writes unconditionally while remembering only on first claim |
| 21 | Every new spec test fails with the exact line it guards deleted | Mutation-run per guard, not per file: the property-ownership guard and the `connectedCallback` re-acquire were each deleted in turn and the matching test turned red, then restored (`git diff` clean, suite back to its full count) |
