...
- Re-gate over the sentinel fixes `f2e9236..a382b4c` (`/code-review xhigh` 15 findings, architecture FORTIFY, tests WARN, maintainability Ready). Fixed after it: `run-all` excuses a report-only script's own `ok` when it returned a summary (`run-all mud-icon --only 16` was exit 1 with `blockers: []`; now exit 0, a crash still fails); sub-component resolution is opt-in (`allowSubComponent`), taken only by 02/04/14/16, so 01/05 and the scaffolds report `mud-tab` not found again instead of demanding its stories and spec; `--changed` in script 16 drops deleted component folders, and its spec compares `componentsScanned` with the changed list (this worktree has no local `main`, so the list is empty here); script 16 treats a spread, a shorthand `shadow` and a quoted `'shadow'` key correctly, reports members after `render()` once whichever comes first, and reads a literal write as a validation fallback only in an `if` branch whose condition is not `true`, parenthesised or not, even when nested; RAW-PIXELS pushes every `(` so `calc((…) - 8px)` stays exempt, and `min()`/`max()`/`clamp()` are reported again; HOST-DISPLAY parses with `postcss` (`@layer` applies, braces in strings, empty stylesheet skipped); `lookaround` honours backslash escapes and reads grep named anywhere on the span's line; P8, `form-reactivity.md` and `version-delta.md` no longer ask for inverting the default; `slot-patterns.md` uses `host!: HTMLMud<Name>Element`; agents link EL1; `pre-pr-check` says report-only errors never block. Each new spec case was run against the `a382b4c` source and failed there. `--all` unchanged (02: RAW-PIXELS 27, HOST-DISPLAY 0; 16: 3 errors / 45 warnings, same codes). CI `Dependencies security audit` on `f2e9236` failed inside `yarn install` (Yarn 4.12 `onCancel handler was attached after the promise settled`, yarnpkg/berry#7245, fix PR #7249 unmerged; the other three jobs installed the same lockfile on the same Node 24.20.0), not in the audit.
- Second re-gate over `a382b4c..99b38e4` (`/code-review xhigh` 15 findings, architecture FORTIFY, tests Review): most above-bar findings sat in that round's own fixes, so the loop was stopped and handed to Dan, who chose a bounded fix list plus push without another round, and retiring `STENCIL-FORM-BOOLEAN-DEFAULT-TRUE` (a string `"false"` turns any form-associated boolean prop on whatever its default, so no component-side change clears it; the fact stays as a consumer note in `version-delta.md` and the two runtime guards in the parity spec). Fixed: `run-all` excuses a report-only script only when it exited 1 with a summary and no STRUCTURE-NOT-FOUND (`run-all mud-does-not-exist --only 16` exit 1 again; `mud-icon` exit 0) and reports that script `ok: true`; a script with no result envelope fails; `pre-pr-check` stops on `ok: false` too; `lib/changed-components.mjs` falls back to `origin/main` and drops deleted folders, and scripts 01/02/04/05 import it instead of four local copies (`--changed` here now finds 7 components); RAW-PIXELS reads `calc` case-insensitively and with a vendor prefix, and the innermost named function decides; `lookaround` reads grep only before the span; `isComponentFile` treats an unreadable entry as not a component; CLI test that 02/04/14/16 accept `mud-tab` and 01/05 do not. Not changed, on record: script 14 skips a quoted `'shadow'` key (no component uses one); `--changed` sub-component files are read only by script 16. `--all`: 02 RAW-PIXELS 27, HOST-DISPLAY 0; 16: 3 errors / 40 warnings (the 5 retired warnings gone). `run-all --changed --only 02,04,14,16` now scans the changed components and reports 2 pre-existing blockers outside this plan's scope: `ANTIPATTERN-025-EVENT-PREFIX` `mud-accordion-item.tsx:220`, `ANTIPATTERN-001-INLINE-STYLE` `mud-table.tsx:454`.
