...
That must list the files still holding physical references — after Phase 3 it prints four, totalling ten. A clean specifier grep beside an empty control is a check that examined nothing.
...
7. **The `scripts/` suite does not run in CI.** `pipline-mud-publish-npm.yml` runs `tokens.build`, `build`, `build.web`, `test.dev`, `validate.package`. `test:scripts` is not among them. That is why Phase 2 puts the new check inside `validate-package.mjs` itself rather than only in the test suite.
