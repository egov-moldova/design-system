...
**1. Does the fix reuse the defect's own mechanism class?** The defect is a build step silently overwriting another step's output. `no instance` for the honesty-reliance half — the gate asks no actor to self-report; `packedFileList()` asks the packer itself. Its inverse is paired, not assumed: Task 6 Step 6 reintroduces the real defect (`stencil build --dev`) and requires the gate to go red. A gate never seen to fail is not a gate.
  ...
  **One residual, stated rather than waved past.** An earlier draft of this row claimed "the instrument and the published artifact cannot disagree". That was false as written and is withdrawn. The gate measures `yarn pack --dry-run`; the pipeline publishes with `npm publish` (`pipline-mud-publish-npm.yml:53`), which is a different tool operating at a later moment. Two things bound the residual, and neither is an argument that it is zero: the two packers were compared on this exact package and produced **identical** file lists — and that comparison is no longer a one-time measurement: `checkPackerAgreement` in `scripts/validate-package.mjs` re-derives it on every gate run and fails first, before any verdict that reads the yarn list — and the gate is anchored as the last step before publish, so nothing in the pipeline writes between them. The window is real and is accepted, not closed. Closing it would mean packing a real tarball and publishing that artifact (`npm publish <tarball>`), which changes what `npm publish` does on the release path — a larger change than the window justifies today.
...
The gate measures `yarn pack`; the pipeline publishes with `npm publish`. They agree on this package today — and that agreement is what makes a yarn-measured verdict a statement about an npm-published tarball, so it is checked, not assumed.

The check ships inside the gate as `checkPackerAgreement` (`scripts/validate-package.mjs`), graded first so a divergence fails before any category that reads the yarn list. The shell block below is the same comparison by hand, for diagnosing a failure outside CI:
