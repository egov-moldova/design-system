# Plan de implementare — `cor-icon` lazy-load via assets

## Context

După build, `dist/esm/cor-icon.entry.js` are **528 KB** — 96% din masă vine dintr-un singur fișier: [src/components/cor-icon/assets/icons.registry.ts](x:/WORK/corlab/age-design/src/components/cor-icon/assets/icons.registry.ts) (516 KB). Registry-ul este un obiect TypeScript care conține markup-ul SVG inline pentru toate 195 iconițe × până la 4 mărimi (12/16/20/24).

[cor-icon.providers.ts:2](x:/WORK/corlab/age-design/src/components/cor-icon/cor-icon.providers.ts#L2) face un **import static** al întregului registry, iar [cor-icon.tsx:91](x:/WORK/corlab/age-design/src/components/cor-icon/cor-icon.tsx#L91) face lookup prin `registry[name]` — indexare dinamică pe care bundlerele nu o pot tree-shake. Orice consumator al `<cor-icon>` primește toate 195 iconițele, indiferent dacă folosește doar una.

**Pragul de avertizare la audit** = 50 KB. cor-icon este la **10.5× pragul**.

**Outcome dorit:**
- `dist/esm/cor-icon.entry.js` să scadă la **~5–8 KB** (componenta + manifest)
...
Pattern-ul este deja folosit intern în [src/legacy/cor-illustration/cor-illustration.tsx:10,62](x:/WORK/corlab/age-design/src/legacy/cor-illustration/cor-illustration.tsx#L10) — există precedent verificat. `.storybook/main.mjs` are deja config pentru rezolvarea path-urilor (linia 25–34).

### Arhitectura propusă

```
┌─────────────────────────────────────────────────────────────┐
│ <cor-icon name="search" size="20">                          │
└──────────────────────────┬──────────────────────────────────┘
...
1. **Manifestul rămâne importat static** ([icons.manifest.json](x:/WORK/corlab/age-design/src/components/cor-icon/assets/icons.manifest.json), 12 KB). Trebuie cunoscut sincron pentru:
...
3. **NU folosim `<img src>`** — pierde `fill: var(--icon-color)` din CSS (linia 31, [cor-icon.css](x:/WORK/corlab/age-design/src/components/cor-icon/cor-icon.css)), iar cele 8 token-uri de culoare din demo nu mai funcționează.
...
### 1. `src/components/cor-icon/cor-icon.tsx` — restructurare async
- Adaugă `assetsDirs: ['assets']` la decoratorul `@Component`
- Importă `getAssetPath` din `@stencil/core`
- Înlocuiește `resolveIcon` sincron cu un flow async (pattern din `cor-illustration`):
...
### 2. `src/components/cor-icon/cor-icon.providers.ts` — rescriere
...
### 3. `src/components/cor-icon/cor-icon.types.ts` — adăugări
...
### 4. `src/components/cor-icon/assets/icons.registry.ts` — **ștergere**
...
`assetsDirs` la nivel de `@Component` este suficient. Stencil 4.x copiază automat `src/components/cor-icon/assets/` → `dist/design-system/assets/{12,16,20,24}/`. Nu necesită config global suplimentar.

### 7. `.storybook/main.mjs` — activare staticDirs
La linia 30–36, există blocul `staticDirs` cu un comentariu explicit despre cor-illustration dezactivat în migrare. Adaugă mapping pentru cor-icon:
```js
staticDirs: [
  { from: '../tokens/generated', to: 'tokens/generated' },
  { from: '../src/components/cor-icon/assets', to: 'assets/assets' }, // nou
...
### 8. `src/components/cor-icon/test/cor-icon.spec.tsx` — adaptare la fetch
- 3 modificări:
...
### 9. `src/components/cor-icon/cor-icon.stories.ts` — nicio modificare funcțională
...
API-ul declarativ rămâne identic (`<cor-icon name="search" size="12">`).

### 11. `src/index.ts` — pregătire pentru export public
Acest plan **nu adaugă** `cor-icon` în barrel-ul public (rămâne import direct prin path-ul componentului, conform [src/index.ts:5](x:/WORK/corlab/age-design/src/index.ts#L5) — doar `cor-spinner` este exportat momentan). Bundle-ul redus permite însă exportul în viitor fără penalizare.
...
|------|------|------------|
| **F1 — Provider** | Rescrie `cor-icon.providers.ts` cu `resolveIconAsset` + `fetchIconSvg` + cache. Adaugă tipul `IconManifest`. | Unit tests pure pe `resolveIconAsset` (fallback up/down, unknown name → undefined) |
...
| **F3 — Storybook config** | Activează `staticDirs` pentru `src/components/cor-icon/assets` în `.storybook/main.mjs`. | `yarn storybook` → toate variantele se randează vizual |
...
| **F5 — Tests** | Actualizează `cor-icon.spec.tsx` cu mock fetch + teste de cache + 404 handling. | `yarn test cor-icon` → toate testele trec |
| **F6 — Verificare bundle** | `yarn build` (Stencil prod). Măsoară `dist/esm/cor-icon.entry.js`. | Așteptat: **< 20 KB** (vs 528 KB initial) |
...
| Risc | Mitigare |
|------|----------|
| **FOUC** (Flash of unstyled content): la prima încărcare a unei iconițe, există un interval în care `<cor-icon>` este gol. | Acceptabil pentru icon-uri (sunt mici). Putem adăuga un `min-width/min-height` din CSS (deja există via `--icon-size`) → previne layout shift. |
...
1. **Build size (criteriul principal):**
   ```pwsh
   yarn build
   Get-ChildItem dist/esm/cor-icon.entry.js | Select-Object Name, @{N='KB';E={[math]::Round($_.Length/1KB,1)}}
   ```
   - Înainte: ~528 KB
...
2. **Asset deployment:**
   ```pwsh
   Get-ChildItem -Recurse dist/design-system/cor-icon/assets/16/*.svg | Measure-Object | Select-Object Count
   ```
...
|--------|---------------|
| [src/components/cor-icon/cor-icon.tsx](x:/WORK/corlab/age-design/src/components/cor-icon/cor-icon.tsx) | Rescriere async (componentWillLoad, @Watch, @State) |
| [src/components/cor-icon/cor-icon.providers.ts](x:/WORK/corlab/age-design/src/components/cor-icon/cor-icon.providers.ts) | Rescriere: `resolveIconAsset` + `fetchIconSvg` + cache |
| [src/components/cor-icon/cor-icon.types.ts](x:/WORK/corlab/age-design/src/components/cor-icon/cor-icon.types.ts) | Adaugă `IconManifest`, `IconManifestEntry` |
| [src/components/cor-icon/assets/icons.registry.ts](x:/WORK/corlab/age-design/src/components/cor-icon/assets/icons.registry.ts) | **Ștergere** |
| [scripts/icons/build-registry.mjs](x:/WORK/corlab/age-design/scripts/icons/build-registry.mjs) | Simplificare: doar manifest |
| [.storybook/main.mjs](x:/WORK/corlab/age-design/.storybook/main.mjs) | Activează `staticDirs` pentru cor-icon |
| [src/components/cor-icon/test/cor-icon.spec.tsx](x:/WORK/corlab/age-design/src/components/cor-icon/test/cor-icon.spec.tsx) | Mock fetch + 2 teste noi (cache, 404) |

**Nemodificate** (validare):
- `cor-icon.css` — stilurile funcționează identic cu SVG injectat în Shadow DOM
- `cor-icon.stories.ts` — folosește deja manifestul
- `src/utils/svg-sanitizer.ts` — sanitizer-ul rămâne relevant pentru fetch + innerHTML
- `src/index.ts`, `src/components/index.ts` — cor-icon nu este încă exportat public
