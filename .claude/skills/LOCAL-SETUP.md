...
| `stencil-compliance` | Script-first run procedure over the pinned Stencil version — decorators, lifecycle, host, JSX/styling, form-associated, reactive data, serialization. | No | Active — invoked by `/audit-component @mud-<name> --deep`, `audit-production` Phase 1, and Wave 1 grep gates in `/pre-pr-check`. |
