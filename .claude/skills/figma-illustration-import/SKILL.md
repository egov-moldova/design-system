...
description: Use when a Figma design requires a custom multi-layer illustration rather than a standard single-glyph icon. Check for an existing `mud-illustration-*` component first, then extract the Figma node, create a reusable Stencil illustration component, add stories, and verify it visually in Storybook.
---
...
1. Check whether the illustration already exists in `src/components/mud-illustration-*/`.
2. If it does not exist, extract the Figma node, screenshot, and variables.
3. Create a reusable `mud-illustration-*` Stencil component.
...
- Use Shadow DOM and a dedicated `mud-illustration-*` component.
...
- `src/components/mud-illustration-*/`
