...
| Component | What it is | When to use |
|---|---|---|
| `MudTextInput` | Text input with label, helper text, error state, optional icons, variants. | Any plain text field. Names, identifiers, references. |
| `MudNumericInput` | Numeric input with optional steppers, min/max/step/precision. | Quantities, money, counts. **Not** for numeric codes (use `MudTextInput` to preserve leading zeros). |
| `MudTextarea` | Multi-line text input with optional character counter. | Long-form text — comments, justifications, reasons. |
| `MudPhoneInput` | Phone field with country selector and masking. | Phone numbers. Never hand-roll masking. |
| `MudDateInput` | Date entry field (typed input, no calendar). | Date fields without picker. |
| `MudDatePicker` | Calendar picker (single date or range). | Date fields with picker, range selection (e.g. report interval). |
| `MudSelect` | Dropdown with options array. | Single-select dropdowns. Language switchers, scope selectors. |
| `MudCheckbox` | Tri-state checkbox (`checked` / `indeterminate` / unchecked). | Boolean form fields, bulk-select-all on tables. |
| `MudRadio` | Single radio (use a group via `name` for mutually-exclusive choice). | Mutually-exclusive choice from a small set. |
| `MudSwitch` | On/off toggle. | Settings toggles. **Not** for form fields that need a submit step — use `MudCheckbox`. |
| `MudFileInput` + `MudFileItem` | File picker with drop zone + per-file display (idle/loading/error/done). | Uploads. Excel imports. Documents. |
| `MudSearchInput` | Search input, `shape="rectangular"` (form-field look) or `shape="circular"` (pill). | Search bars in lists/tables/filter rails; top-nav or hero search. |
| `MudInputChip` | Input that converts entries to chips on `Enter` or separator. | Tag entry, comma-separated values, "type and tag". |
| `MudSegmentedControl` | Mutually-exclusive segments. | Small choice with ≤4 options where dropdown feels heavy. Time-range toggles, density toggles. |
...
| Component | What it is | When to use |
|---|---|---|
| `MudTable` | Data table with sort, select, hover, configurable header/row styles. | Every list view with structured rows. **Don't** roll a `<table>` by hand. |
| `MudTag` | Static label (neutral / semantic variants like positive/danger/warning/info). | Status badges on table rows. State markers next to inline content. |
| `MudChip` | Compact pill — `type="filter"` (toggle, emits `mudSelect`) or `type="input"` (static value, optionally `removable`). | Active-filter chips on a filter rail. Role chips, tag tokens, removable selections. |
| `MudBadge` | Tiny count or dot on icons / nav items. | Notification beacon on nav items. Unread counts. |
| `MudAvatar` | Initials, image, or icon avatar. Sizes from xs to lg. | User menus, audit logs, comment authors. |
...
1. **Hand-rolling a button.** `<button class="primary">` is a code smell. `MudButton`.
2. **Hand-rolling a tab strip.** `<nav><a class="active">` is a code smell. `MudTabs` + `MudTab` with `value`/`onMudChange`.
3. **Hand-rolling a chip / tag.** `<span class="badge">` is a code smell. `MudChip` (interactive / toggleable) or `MudTag` (static).
4. **Inline SVGs.** `<svg>...</svg>` in JSX is a code smell. `MudIcon name="…"` resolves from the registry.
