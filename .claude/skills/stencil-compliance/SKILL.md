...
| @Prop | P8: On a form-associated component, a boolean prop defaulting to `true` is reported: a string `"false"` assigned to the property parses as `true` there, so a consumer that sets the property from a template string cannot turn it off (HTML attributes are coerced to a boolean first). The fix is consumer-side, a boolean value; inverting the default does not help | `script-16:STENCIL-FORM-BOOLEAN-DEFAULT-TRUE` | `STENCIL-FORM-BOOLEAN-DEFAULT-TRUE` |
