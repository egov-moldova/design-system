...
- **Auto-generated files are read-only.** Never hand-edit `src/components.d.ts`, `src/components/*/readme.md`, `.storybook/custom-elements.json`, or `tokens/generated/**` — they're regenerated by `yarn build`. See `_agents/generated-files.md` for how conflicts in these files are resolved.
...
Husky hooks are installed automatically by `yarn install` — the private `react` workspace's `postinstall` runs `scripts/git/install-hooks.mjs`, so the published `@egov-moldova/mud` manifest carries no install script — no manual setup needed. Bypass once with `git commit --no-verify` / `git push --no-verify`, or `HUSKY=0` for a whole shell.
