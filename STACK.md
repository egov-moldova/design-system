...
- **React / Vue / Angular as the internal framework.** We compile *to* standard web components — we don't bind to any single framework internally. Lit is allowed only inside Storybook templates (`*.stories.ts`), never inside `cor-*` source.
...
- **State management libraries (Redux / MobX / Zustand / XState).** `cor-*` components are stateless or props-driven. Internal state uses `@State`; cross-component coordination happens via `@Event` dispatch to the consumer.
