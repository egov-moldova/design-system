...
| Story missing from sidebar | Wrong `title` or extension | Verify `title: 'Components/MudButton'`, file ends `.stories.ts` |
...
- **Use Cases**: Quick iterations, prototypes, experimental features (single atoms/molecules
  only — see `_agents/workflow-rules.md`)

Git & PR workflow (below) applies regardless of `--fast` — there is no flag that gates it.

---

## Staging

Stage paths explicitly (`git add <paths>`) — never `git add -A` or `git add .`. Tracked
generated files (component/hidden `readme.md`) are regenerated by
`yarn build` and committed together with the source change that regenerated them, never
hand-edited or staged separately. `src/components.d.ts`, `.storybook/custom-elements.json` and `tokens/generated/**`
are git-ignored — they are never staged at all. Full mechanism: `_agents/generated-files.md`.

---

## Change Scope Discipline (always active)
...
What the last 36 PRs actually did (`gh pr list -R egov-moldova/design-system --state all -L 40`),
not an enforced house style — there is no branch-name or PR-title lint.

- **Commits**: Conventional Commits (`feat:`, `fix:`, `docs:`, `refactor:`, `test:`, `chore:`,
  `build:`, `style:`, a `!` suffix for a breaking change) — enforced by the `commit-msg` hook
  (commitlint). This is the one convention that is actually checked.
- **Branch**: `type/issue-NN-slug` (`fix/issue-53-ai-docs-alignment`) is the dominant form; a
  numeric ticket in place of `issue-NN` (`fix/93678-toast-styles`) and a bare `type/slug`
  (`chore/pixel-perfect-pipeline`) both occur. No fixed grammar is enforced.
- **PR title**: usually mirrors the primary commit's Conventional Commit form
  (`fix(accordion-item): …`), but a plain-English title lands too when it reads better to a
  reviewer. There is no structured `TYPE :: KEY :: TITLE` naming rule for PR titles — 0 of the
  last 36 used one — and no `[mountain]`/`[boulder]` review-tag convention is in use anywhere in
  the repo.
...
- [ ] Conventional commits used throughout (commitlint will otherwise reject the commit)
