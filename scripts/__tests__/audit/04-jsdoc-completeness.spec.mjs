 ...
 *
 * Strategy:
 *   - Sanity-scan our 3 baseline components (cor-button, cor-input, cor-tooltip) —
 ...
 *   - 0 error-level findings on cor-button, cor-input, cor-tooltip.
...
describe('04-jsdoc-completeness: baseline regression', () => {
  it('cor-button has zero error-level findings', async () => {
    ...
    assert.ok(api.props > 0, 'expected at least one prop on cor-button');
  });

  it('cor-input and cor-tooltip both produce 0 errors', async () => {
...
describe('04-jsdoc-completeness: prop checks', () => {
  it('flags @Prop() without JSDoc as warning (JSDOC-PROP-MISSING)', () => {
    ...
    const { findings } = analyzeTsxFile(p, 'cor-test');
...
describe('04-jsdoc-completeness: component-level checks', () => {
  ...
  it('errors when file has no @Component class', () => {
    ...
    const { findings } = analyzeTsxFile(p, 'cor-fake');
