...
const FORBIDDEN_PATH_PATTERNS = [
  ...
  {
    // Consumers get the declarations from the published package (dist/types/components.d.ts),
    // so a tracked copy serves no one and conflicts on every parallel PR.
    pattern: /^src\/components\.d\.ts$/,
    code: 'GIT-STAGED-COMPONENTS-DTS',
    message: '`src/components.d.ts` is regenerated by the build and git-ignored; do not force-add it.',
  },
  {
...
function getStagedFiles() {
  // Deletions excluded: removing a forbidden path from the index is the cleanup, not a breach.
  const res = spawnSync('git', ['diff', '--name-only', '--cached', '--diff-filter=d'], { encoding: 'utf8' });
  if (res.status !== 0) return [];
