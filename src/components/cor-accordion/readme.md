...
| `iconPosition`   | `icon-position`   | Trigger-icon placement relative to the header content. Set by the parent `cor-accordion`.                                                                                                                                                                                      | `"left" \| "right"`          | `'right'`   |
...
| `size`           | `size`            | Visual size rung — controls header height, font size, icon size, padding. Set by the parent `cor-accordion` via `size`; consumers should configure size at the container level.                                                                                                | `"md" \| "sm"`               | `'md'`      |
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| `corAccordionItemKey` | Emitted on Arrow/Home/End keypress on the header. Consumed by the parent `cor-accordion` to implement WAI-ARIA Accordion Pattern traversal. Internal contract — consumers typically don't subscribe directly. | `CustomEvent<{ key: string; itemId: string; }>`   |
