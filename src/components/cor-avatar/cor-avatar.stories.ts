...
import { AVATAR_SIZES, AVATAR_TYPES } from './cor-avatar.types';
...
const renderAvatar = (args: AvatarArgs) => /*html*/ `
  ...
  >${renderBadgeChild(args.badge)}</cor-avatar>
`;
...
const renderAllVariants = () => /*html*/ `
  <div style="${wrapStyle}">
    ...
    <div style="${cellWrapStyle}">
      <cor-avatar type="initials" size="md" name="Maria Pop"></cor-avatar>
      <span style="${cellLabelStyle}">initials</span>
    </div>
    <div style="${cellWrapStyle}">
      <cor-avatar type="icon" size="md" aria-label="User"></cor-avatar>
      ...
      <div style="${wrapStyle}">
        ${AVATAR_SIZES.map(
          ...
          <div style="${cellWrapStyle}">
            ${
              type === 'photo'
                ? /*html*/ `<cor-avatar type="photo" size="${size}" src="${SAMPLE_PHOTO_MARIA}" name="Maria Pop"></cor-avatar>`
                : type === 'initials'
                  ? /*html*/ `<cor-avatar type="initials" size="${size}" name="Ion Popescu"></cor-avatar>`
                  : /*html*/ `<cor-avatar type="icon" size="${size}" aria-label="User"></cor-avatar>`
            }
...
const renderStates = () => /*html*/ `
  ...
  <div class="state-grid">
    ...
    <cor-avatar type="photo" size="md" src="${SAMPLE_PHOTO_ION}" name="Ion Popescu" class="is-focus-demo" tabindex="0"></cor-avatar>
    <cor-avatar type="initials" size="md" name="Maria Pop" class="is-focus-demo" tabindex="0"></cor-avatar>
    <cor-avatar type="icon" size="md" aria-label="User" class="is-focus-demo" tabindex="0"></cor-avatar>
  </div>
...
const renderWithBadge = () => /*html*/ `
  ...
  <div style="${wrapStyle}">
    <div style="${cellWrapStyle}">
      <cor-avatar type="initials" size="md" name="Ion Popescu">
    ...
    <div style="${cellWrapStyle}">
      <cor-avatar type="photo" size="md" src="${SAMPLE_PHOTO_MARIA}" name="Maria Pop">
        <span slot="badge" class="demo-badge demo-badge--dot" aria-label="Online"></span>
      </cor-avatar>
      <span style="${cellLabelStyle}">dot badge</span>
    </div>
    <div style="${cellWrapStyle}">
      <cor-avatar type="icon" size="lg" aria-label="System notifications">
  ...
  <div style="display: flex; flex-direction: column; gap: var(--spacing-24); padding: var(--spacing-24);">
    <div>
      ...
      <div class="avatar-stack">
        <cor-avatar type="photo" size="md" src="${SAMPLE_PHOTO_ION}" name="Ion Popescu"></cor-avatar>
        <cor-avatar type="photo" size="md" src="${SAMPLE_PHOTO_MARIA}" name="Maria Pop"></cor-avatar>
        <cor-avatar type="photo" size="md" src="${SAMPLE_PHOTO_ANDREI}" name="Andrei Ionescu"></cor-avatar>
      </div>
    </div>
    ...
    <div>
      ...
      <div class="avatar-stack">
        <cor-avatar type="photo" size="md" src="${SAMPLE_PHOTO_ELENA}" name="Elena Dumitrescu"></cor-avatar>
        <cor-avatar type="initials" size="md" name="Vlad Georgescu"></cor-avatar>
        <cor-avatar type="icon" size="md" aria-label="Invitație în așteptare"></cor-avatar>
      </div>
    </div>
  </div>
`;
  ...
  <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: var(--spacing-16); padding: var(--spacing-24);">
    ${ROMANIAN_TEAM.map(
      ...
      <div style="display: flex; align-items: center; gap: var(--spacing-12); padding: var(--spacing-12); background: var(--color-background-base-default); border: 1px solid var(--color-border-base-subtle); border-radius: var(--border-radius-8);">
        <cor-avatar type="photo" size="md" src="${person.photo}" name="${person.name}"></cor-avatar>
    ...
    ${ROMANIAN_TEAM.map(
      person => /*html*/ `
      <div style="display: flex; align-items: center; gap: var(--spacing-12); padding: var(--spacing-12); background: var(--color-background-base-default); border: 1px solid var(--color-border-base-subtle); border-radius: var(--border-radius-8);">
        <cor-avatar type="initials" size="md" name="${person.name}"></cor-avatar>
        <div style="display: flex; flex-direction: column;">
          <span style="font-family: var(--font-family-primary); font-weight: var(--font-weight-medium); font-size: var(--font-size-14); color: var(--color-text-base-default);">${person.name}</span>
...
const docsSourceDefault = (args: AvatarArgs) => {
  const attrs = [
    ...
    .filter(Boolean)
    .join(' ');
  const open = attrs ? `<cor-avatar ${attrs}>` : '<cor-avatar>';
...
const docsSourceAllSizes = AVATAR_SIZES.map(s => `<cor-avatar size="${s}" name="Ion Popescu"></cor-avatar>`).join('\n');

const docsSourceWithBadge = /*html*/ `<cor-avatar type="initials" name="Ion Popescu">
  <span slot="badge" class="demo-badge demo-badge--count" aria-label="3 notificări noi">3</span>
</cor-avatar>

<cor-avatar type="photo" src="…" name="Maria Pop">
  <span slot="badge" class="demo-badge demo-badge--dot" aria-label="Online"></span>
</cor-avatar>`;

const docsSourceStack = /*html*/ `<!-- Avatar stack composition (no dedicated component) -->
<div class="avatar-stack">
  <cor-avatar type="photo" src="…" name="Ion Popescu"></cor-avatar>
  <cor-avatar type="photo" src="…" name="Maria Pop"></cor-avatar>
  <cor-avatar type="photo" src="…" name="Andrei Ionescu"></cor-avatar>
  <span class="stack-more" aria-hidden="true">+2</span>
</div>
...
const docsSourceRomanianNames = /*html*/ `<cor-avatar type="photo" src="…" name="Ion Popescu"></cor-avatar>
<cor-avatar type="initials" name="Maria Pop"></cor-avatar>
<cor-avatar type="initials" name="Andrei Ionescu"></cor-avatar>
<cor-avatar type="initials" name="Elena Dumitrescu"></cor-avatar>
<cor-avatar type="initials" name="Vlad Georgescu"></cor-avatar>`;
...
export const States: Story = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        story:
          'Two-row matrix mirroring Figma 203:2111. Avatars are non-interactive by default; the `focus` row demonstrates the ring that appears when a consumer makes the avatar focusable (e.g. wraps it in a `<cor-button>`).',
      },
    },
  },
};
