...
import { deriveInitials, ICON_SIZE_FOR } from '../cor-avatar.utils';

// `cor-icon` fetches its SVGs asynchronously via `getAssetPath` + `fetch`.
...
const queryIcon = (root: Element | null | undefined): Element | null =>
  (root?.shadowRoot?.querySelector('cor-icon') ?? null) as Element | null;
...
describe('ICON_SIZE_FOR', () => {
  it('maps every avatar size to a supported cor-icon size', () => {
...
describe('cor-avatar', () => {
  it('renders with default props reflected on host', async () => {
    const { root } = await render(<cor-avatar name="Ion Popescu"></cor-avatar>);
  ...
  describe('type prop', () => {
    it.each(AVATAR_TYPES)('reflects type="%s" to the host attribute', async type => {
      const { root } = await render(
        <cor-avatar type={type} src="https://example.com/a.png" name="Ion Popescu"></cor-avatar>,
      );
  ...
  describe('size prop', () => {
    it.each(AVATAR_SIZES)('reflects size="%s" to the host attribute', async size => {
      const { root } = await render(<cor-avatar size={size} name="Ion Popescu"></cor-avatar>);
  ...
  describe('type="initials" rendering', () => {
    it('renders 2-letter initials derived from `name`', async () => {
      const { root } = await render(<cor-avatar type="initials" name="Ion Popescu"></cor-avatar>);
    ...
    it('prefers explicit `initials` over `name`', async () => {
      const { root } = await render(<cor-avatar type="initials" name="Ion Popescu" initials="AB"></cor-avatar>);
      expect(queryInitials(root)?.textContent).toBe('AB');
    });

    it('trims initials to two characters', async () => {
      const { root } = await render(<cor-avatar type="initials" initials="ABCDE"></cor-avatar>);
      expect(queryInitials(root)?.textContent).toBe('AB');
    });

    it('uppercases lowercase initials', async () => {
      const { root } = await render(<cor-avatar type="initials" initials="ab"></cor-avatar>);
      expect(queryInitials(root)?.textContent).toBe('AB');
    });
    ...
    it('falls back to the icon when neither name nor initials are provided', async () => {
      const { root } = await render(<cor-avatar type="initials" aria-label="User"></cor-avatar>);
  ...
  describe('type="photo" rendering', () => {
    it('renders an <img> with src and alt', async () => {
      const { root } = await render(
        <cor-avatar type="photo" src="https://example.com/ion.png" name="Ion Popescu"></cor-avatar>,
      );
    ...
    it('accepts an empty alt to mark the photo as decorative', async () => {
      const { root } = await render(<cor-avatar type="photo" src="https://example.com/a.png" alt=""></cor-avatar>);
      expect(queryPhoto(root)?.getAttribute('alt')).toBe('');
    });

    it('falls back to initials when `src` is missing', async () => {
      const { root } = await render(<cor-avatar type="photo" name="Ion Popescu"></cor-avatar>);
    ...
    it('falls back to the icon when `src` is missing AND no name', async () => {
      const { root } = await render(<cor-avatar type="photo" aria-label="User"></cor-avatar>);
      expect(queryPhoto(root)).toBeNull();
      expect(queryIcon(root)).toBeTruthy();
    });
  });
  ...
  describe('type="icon" rendering', () => {
    it('renders the default person icon', async () => {
      const { root } = await render(<cor-avatar type="icon" aria-label="User"></cor-avatar>);
      const icon = queryIcon(root) as (Element & { name?: string }) | null;
      expect(icon).toBeTruthy();
      // `name` is not a reflected attribute on cor-icon — assert via the JS property.
      expect(icon?.name).toBe('person');
    });

    it('respects a custom icon-name', async () => {
      const { root } = await render(<cor-avatar type="icon" iconName="shield-user" aria-label="Admin"></cor-avatar>);
      const icon = queryIcon(root) as (Element & { name?: string }) | null;
      expect(icon?.name).toBe('shield-user');
    });

    it.each(AVATAR_SIZES)('scales the icon to the right size for size="%s"', async size => {
      const { root } = await render(<cor-avatar type="icon" size={size} aria-label="User"></cor-avatar>);
  ...
  describe('accessibility', () => {
    ...
    it('uses `aria-label` override when provided', async () => {
      const { root } = await render(
        <cor-avatar name="Ion Popescu" ariaLabel="Coleg cu rol de administrator"></cor-avatar>,
      );
  ...
  describe('badge slot', () => {
    it('renders a slotted badge child', async () => {
      const { root } = await render(
        <cor-avatar type="initials" name="Ion Popescu">
  ...
  describe('shadow structure', () => {
    ...
    it('applies the resolved-mode class on `.inner` (photo)', async () => {
      const { root } = await render(
        <cor-avatar type="photo" src="https://example.com/a.png" name="Ion Popescu"></cor-avatar>,
      );
