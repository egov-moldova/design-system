...
import type { CheckboxChangeDetail, CheckboxSize } from './cor-checkbox.types';
...
export class CorCheckbox {
  ...
  @Element() host!: HTMLCorCheckboxElement;

  @AttachInternals() internals!: ElementInternals;

  /** Fires when `checked` (or `indeterminate`) changes from a user action. */
  @Event() corChange!: EventEmitter<CheckboxChangeDetail>;
  ...
  private readonly labelId = `cor-checkbox-label-${this.instanceId}`;
  private readonly supportingId = `cor-checkbox-supporting-${this.instanceId}`;
  ...
  validateSize(next: CheckboxSize) {
    if (!CHECKBOX_SIZES.includes(next)) {
      console.warn(
        `[cor-checkbox] size="${String(next)}" is not supported. Supported: ${CHECKBOX_SIZES.join(
          ', ',
        )}. Falling back to "md".`,
      );
      this.size = 'md';
  ...
  private handleChange = (ev: Event) => {
    ...
    this.corChange.emit({ checked: this.checked, indeterminate: this.indeterminate, value: this.value });
  };
