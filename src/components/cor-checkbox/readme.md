...
| `label`          | `label`           | Accessible-name fallback. Used as `aria-label` on the internal input when no `label` slot is provided. Does NOT render visible text — use the `label` slot for that. Matches the `cor-button` convention. | `string \| undefined` | `undefined` |
...
| ----------- | ------------------------------------------------------------------------------- | ----------------------------------- |
| `corBlur`   | Fires when the control loses focus. The native `FocusEvent` is forwarded as-is. | `CustomEvent<FocusEvent>`           |
| `corChange` | Fires when `checked` (or `indeterminate`) changes from a user action.           | `CustomEvent<CheckboxChangeDetail>` |
| `corFocus`  | Fires when the control gains focus. The native `FocusEvent` is forwarded as-is. | `CustomEvent<FocusEvent>`           |
