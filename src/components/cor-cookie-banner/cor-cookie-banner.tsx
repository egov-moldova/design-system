 ...
 *                    `cor-switch` wiring and the resulting consent payload).
 */
@Component({
  tag: 'cor-cookie-banner',
  styleUrl: 'cor-cookie-banner.css',
  shadow: true,
})
export class CorCookieBanner {
  /**
   * Layout flavour.
  ...
  private readonly titleId = `cor-cookie-banner-title-${this.instanceId}`;

  componentWillLoad() {
    if (!COOKIE_BANNER_VARIANTS.includes(this.variant)) {
      console.warn(
        `[cor-cookie-banner] Invalid variant="${String(this.variant)}". ` +
    ...
    if (!COOKIE_BANNER_POSITIONS.includes(this.position)) {
      console.warn(
        `[cor-cookie-banner] Invalid position="${String(this.position)}". ` +
  ...
  handleVariantChange(next: CookieBannerVariant) {
    if (!COOKIE_BANNER_VARIANTS.includes(next)) {
      console.warn(
        `[cor-cookie-banner] Invalid variant="${String(next)}". ` +
          `Expected one of ${COOKIE_BANNER_VARIANTS.join(', ')}. Falling back to 'simple'.`,
      );
      this.variant = 'simple';
    }
  ...
  handlePositionChange(next: CookieBannerPosition) {
    if (!COOKIE_BANNER_POSITIONS.includes(next)) {
      console.warn(
        `[cor-cookie-banner] Invalid position="${String(next)}". ` +
          `Expected one of ${COOKIE_BANNER_POSITIONS.join(', ')}. Falling back to 'bottom'.`,
      );
      this.position = 'bottom';
    }
  ...
  private handleCategoryToggle = (id: string, ev: Event) => {
    const target = ev.target as HTMLCorSwitchElement | null;
  ...
  private handleRejectClick = (ev: Event) => {
    ...
    this.corReject.emit(payload);
  };

  private handleSaveClick = (ev: Event) => {
    ev.preventDefault();
    this.corSavePreferences.emit(this.buildCurrentSelectionPayload());
  };
    ...
    return (
      <div class="header" part="header">
        ...
        {showClose ? (
          ...
          >
            <cor-icon name="chevron-bottom-medium" size={16} aria-hidden="true" />
          </button>
        ) : null}
      </div>
    );
        ...
        <p class="body-text" hidden={this.hasBodySlot}>
          ...
          {this.privacyHref ? (
            <span class="privacy-wrapper">
              {' '}
              <cor-link href={this.privacyHref} size="sm" variant="primary">
      ...
      <div class="category" part="category" data-category-id={cat.id}>
        <div class="category-text">
          <div class="category-heading">
            ...
            {cat.required ? (
              <cor-tag size="sm" type="outlined" semantic="brand" label={COOKIE_BANNER_DEFAULTS.requiredLabel} />
            ) : null}
          </div>
        ...
        {cat.required ? (
          <span class="category-required-icon" aria-hidden="true">
            <cor-icon name="checkmark-large" size={24} aria-hidden="true" />
          </span>
        ) : (
          <cor-switch
            checked={enabled}
            aria-label={cat.label}
            onCorChange={(ev: Event) => this.handleCategoryToggle(cat.id, ev)}
          />
        )}
      </div>,
      !isLast ? <cor-separator orientation="horizontal" /> : null,
    ];
  }
    ...
    if (this.expanded) {
      return (
        <div class="footer footer--expanded" part="footer">
          <cor-button variant="primary" appearance="filled" size="md" onClick={this.handleSaveClick}>
            {save}
          </cor-button>
        </div>
      );
