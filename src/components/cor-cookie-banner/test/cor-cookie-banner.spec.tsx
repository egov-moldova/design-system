...
import '../cor-cookie-banner';

import { COOKIE_BANNER_DEFAULTS, COOKIE_BANNER_POSITIONS, COOKIE_BANNER_VARIANTS } from '../cor-cookie-banner.types';
import type { CookieCategory } from '../cor-cookie-banner.types';
...
describe('cor-cookie-banner', () => {
  describe('defaults', () => {
    it('renders with default props reflected on host', async () => {
      const { root } = await render(<cor-cookie-banner></cor-cookie-banner>);
      ...
      expect(root?.getAttribute('aria-labelledby')).toMatch(/^cor-cookie-banner-title-/);
    });
    ...
    it('renders three buttons in collapsed simple variant', async () => {
      const { root } = await render(<cor-cookie-banner></cor-cookie-banner>);
      const buttons = queryShadowAll(root, 'cor-button');
    ...
    it('does not render categories section when collapsed', async () => {
      const { root } = await render(<cor-cookie-banner variant="detailed"></cor-cookie-banner>);
  ...
  describe('variant prop', () => {
    it.each(COOKIE_BANNER_VARIANTS)('reflects variant="%s" on host', async variant => {
      const { root } = await render(<cor-cookie-banner variant={variant}></cor-cookie-banner>);
    ...
    it('warns and falls back to simple on invalid variant', async () => {
      ...
      const { root } = await render(
        ...
        <cor-cookie-banner variant="bogus"></cor-cookie-banner>,
      );
      await flush();
  ...
  describe('position prop', () => {
    it.each(COOKIE_BANNER_POSITIONS)('reflects position="%s" on host', async position => {
      const { root } = await render(<cor-cookie-banner position={position}></cor-cookie-banner>);
    ...
    it('warns and falls back to bottom on invalid position', async () => {
      ...
      const { root } = await render(
        // @ts-expect-error invalid value used to drive the @Watch fallback
        <cor-cookie-banner position="diagonal"></cor-cookie-banner>,
      );
      await flush();
      expect(warn).toHaveBeenCalled();
      expect(root?.getAttribute('position')).toBe('bottom');
  ...
  describe('label overrides', () => {
    it('honors a custom titleText prop', async () => {
      const { root } = await render(<cor-cookie-banner title-text="Politica noastră"></cor-cookie-banner>);
      expect(queryShadow(root, '.title')?.textContent?.trim()).toBe('Politica noastră');
    });

    it('honors a custom body prop', async () => {
      const { root } = await render(<cor-cookie-banner body="Mesaj scurt."></cor-cookie-banner>);
    ...
    it('uses the Romanian default expanded title when expanded', async () => {
      const { root } = await render(<cor-cookie-banner variant="detailed" expanded></cor-cookie-banner>);
  ...
  describe('expanded state', () => {
    ...
    it('emits corDismiss when close button is clicked', async () => {
      const handler = vi.fn();
      const { root } = await render(
        <cor-cookie-banner variant="detailed" expanded onCorDismiss={handler}></cor-cookie-banner>,
      );
  ...
  describe('events', () => {
    it('emits corAccept with all categories=true on accept click', async () => {
      const handler = vi.fn();
      const { root } = await render(
        <cor-cookie-banner categories={sampleCategories} onCorAccept={handler}></cor-cookie-banner>,
      );
      const acceptBtn = queryShadow(root, 'cor-button.cta-accept');
    ...
    it('emits corReject with only required=true on reject click', async () => {
      const handler = vi.fn();
      const { root } = await render(
        <cor-cookie-banner categories={sampleCategories} onCorReject={handler}></cor-cookie-banner>,
      );
      const rejectBtn = queryShadow(root, 'cor-button.cta-reject');
    ...
    it('emits corExpand and sets expanded=true on manage click', async () => {
      const handler = vi.fn();
      const { root } = await render(<cor-cookie-banner onCorExpand={handler}></cor-cookie-banner>);
      const manageBtn = queryShadow(root, 'cor-button.cta-manage');
    ...
    it('emits corSavePreferences with current selection on save click', async () => {
      const handler = vi.fn();
      const { root } = await render(
        <cor-cookie-banner
          variant="detailed"
          ...
          onCorSavePreferences={handler}
        ></cor-cookie-banner>,
      );
      const saveBtn = queryShadow(root, '.footer--expanded cor-button');
  ...
  describe('categories', () => {
    it('renders one row per non-required category with a switch', async () => {
      const { root } = await render(
        <cor-cookie-banner variant="detailed" expanded categories={sampleCategories}></cor-cookie-banner>,
      );
      const rows = queryShadowAll(root, '.category');
      expect(rows).toHaveLength(2);
      const switches = queryShadowAll(root, 'cor-switch');
    ...
    it('renders the Required tag for required categories', async () => {
      ...
      const tag = queryShadow(root, '.category cor-tag');
    ...
    it('renders a checkmark icon for required categories instead of a switch', async () => {
      ...
      expect(requiredRow?.querySelector('cor-switch')).toBeNull();
    });
  ...
  describe('privacy link', () => {
    it('does not render a link when privacyHref is unset', async () => {
      const { root } = await render(<cor-cookie-banner></cor-cookie-banner>);
      expect(queryShadow(root, '.body cor-link')).toBeNull();
    });

    it('renders a cor-link when privacyHref is set', async () => {
      const { root } = await render(<cor-cookie-banner privacy-href="/privacy"></cor-cookie-banner>);
      const link = queryShadow<HTMLElement & { href?: string }>(root, '.body cor-link');
    ...
    it('uses the Romanian default privacy label', async () => {
      const { root } = await render(<cor-cookie-banner privacy-href="/privacy"></cor-cookie-banner>);
      expect(queryShadow(root, '.body cor-link')?.textContent?.trim()).toBe(COOKIE_BANNER_DEFAULTS.privacyLabel);
    });
  });
  ...
  describe('accessibility', () => {
    ...
    it('honors a custom aria-label', async () => {
      const { root } = await render(<cor-cookie-banner aria-label="Consimțământ cookie-uri"></cor-cookie-banner>);
