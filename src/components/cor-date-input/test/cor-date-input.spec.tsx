...
const queryTrailingIcon = (root: Element | null | undefined): Element | null =>
  root?.shadowRoot?.querySelector('.trailing-icon cor-icon') ?? null;
...
describe('cor-date-input', () => {
  describe('defaults + prop reflection', () => {
    it('renders with default props reflected on host', async () => {
      const { root } = await render(<cor-date-input label="Birthday"></cor-date-input>);
    ...
    it.each(DATE_INPUT_VARIANTS)('reflects variant="%s" to host', async variant => {
      const { root } = await render(<cor-date-input variant={variant} label="x"></cor-date-input>);
      expect(root?.getAttribute('variant')).toBe(variant);
    });

    it.each(DATE_INPUT_SIZES)('reflects size="%s" to host', async size => {
      const { root } = await render(<cor-date-input size={size} label="x"></cor-date-input>);
      expect(root?.getAttribute('size')).toBe(size);
    });

    it.each(DATE_INPUT_FORMATS)('reflects format="%s" to host', async format => {
      const { root } = await render(<cor-date-input format={format} label="x"></cor-date-input>);
  ...
  describe('shadow structure', () => {
    ...
    it('uses a 24px calendar icon for size="lg"', async () => {
      const { root } = await render(<cor-date-input size="lg" label="x"></cor-date-input>);
      const icon = queryTrailingIcon(root);
    ...
    it('adds a required mark when `required` is set', async () => {
      const { root } = await render(<cor-date-input label="x" required></cor-date-input>);
    ...
    it('renders a helper assistive row when `helper-text` is set', async () => {
      const { root } = await render(<cor-date-input label="x" helper-text="Pick a date"></cor-date-input>);
    ...
    it('renders an error assistive row with the error icon when invalid + error-text', async () => {
      const { root } = await render(
        <cor-date-input label="x" invalid error-text="Day must be between 01 and 31"></cor-date-input>,
      );
      const assistive = queryAssistive(root);
    ...
    it('error message takes priority over helper text', async () => {
      const { root } = await render(
        <cor-date-input label="x" invalid helper-text="Hint" error-text="Required"></cor-date-input>,
      );
      const assistive = queryAssistive(root);
  ...
  describe('value + form association', () => {
    ...
    it('emits corInput on each keystroke with masked value + segment + iso', async () => {
      const onInput = vi.fn();
      const { root } = await render(<cor-date-input label="x" onCorInput={onInput}></cor-date-input>);
      const native = queryNative(root)!;
      native.value = '15';
    ...
    it('emits corChange on change (blur) with iso payload when valid', async () => {
      const onChange = vi.fn();
      const { root } = await render(
        <cor-date-input label="x" value="15/04/2025" onCorChange={onChange}></cor-date-input>,
      );
      const native = queryNative(root)!;
    ...
    it('emits corChange with isoValue=null when value is incomplete', async () => {
      const onChange = vi.fn();
      const { root } = await render(<cor-date-input label="x" value="15/04/" onCorChange={onChange}></cor-date-input>);
      const native = queryNative(root)!;
      native.dispatchEvent(new Event('change', { bubbles: true }));
      await flush();
    ...
    it('emits corChange with isoValue=null when the calendar date is impossible', async () => {
      // 31/02 doesn't exist — toIsoValue should reject.
      const onChange = vi.fn();
      const { root } = await render(
        <cor-date-input label="x" value="31/02/2025" onCorChange={onChange}></cor-date-input>,
      );
      const native = queryNative(root)!;
      native.dispatchEvent(new Event('change', { bubbles: true }));
      await flush();
    ...
    it('emits corFocus and corBlur and toggles the is-focused class', async () => {
      ...
      const onBlur = vi.fn();
      const { root } = await render(
        <cor-date-input label="x" onCorFocus={onFocus} onCorBlur={onBlur}></cor-date-input>,
      );
      const native = queryNative(root)!;
      native.dispatchEvent(new FocusEvent('focus'));
      await flush();
  ...
  describe('format variations', () => {
    it('formats MM/DD/YYYY correctly', async () => {
      const onChange = vi.fn();
      const { root } = await render(
        <cor-date-input label="x" format="MM/DD/YYYY" value="04/15/2025" onCorChange={onChange}></cor-date-input>,
      );
      const native = queryNative(root)!;
      native.dispatchEvent(new Event('change', { bubbles: true }));
      await flush();
    ...
    it('formats YYYY-MM-DD correctly', async () => {
      const onChange = vi.fn();
      const { root } = await render(
        <cor-date-input label="x" format="YYYY-MM-DD" value="2025-04-15" onCorChange={onChange}></cor-date-input>,
      );
      const native = queryNative(root)!;
      native.dispatchEvent(new Event('change', { bubbles: true }));
      await flush();
    ...
    it('uses the format pattern as the default placeholder', async () => {
      const { root: rootDmy } = await render(<cor-date-input label="x"></cor-date-input>);
      expect(queryNative(rootDmy)?.getAttribute('placeholder')).toBe('DD/MM/YYYY');
      const { root: rootIso } = await render(<cor-date-input label="x" format="YYYY-MM-DD"></cor-date-input>);
  ...
  describe('min / max bounds', () => {
    it('returns null isoValue when the value falls below min', async () => {
      const onChange = vi.fn();
      const { root } = await render(
        <cor-date-input label="x" value="15/04/2025" min="2025-05-01" onCorChange={onChange}></cor-date-input>,
      );
      const native = queryNative(root)!;
      native.dispatchEvent(new Event('change', { bubbles: true }));
      await flush();
    ...
    it('returns null isoValue when the value rises above max', async () => {
      const onChange = vi.fn();
      const { root } = await render(
        <cor-date-input label="x" value="15/04/2025" max="2025-03-31" onCorChange={onChange}></cor-date-input>,
      );
      const native = queryNative(root)!;
      native.dispatchEvent(new Event('change', { bubbles: true }));
      await flush();
  ...
  describe('disabled + readonly behavior', () => {
    it('passes disabled through to the native input', async () => {
      const { root } = await render(<cor-date-input label="x" disabled></cor-date-input>);
      const native = queryNative(root);
    ...
    it('passes readonly through to the native input', async () => {
      const { root } = await render(<cor-date-input label="x" readonly value="15/04/2025"></cor-date-input>);
      const native = queryNative(root);
  ...
  describe('ARIA contract', () => {
    ...
    it('exposes aria-invalid when invalid', async () => {
      const { root } = await render(<cor-date-input label="x" invalid></cor-date-input>);
    ...
    it('wires aria-describedby to the helper id when helper-text present', async () => {
      const { root } = await render(<cor-date-input label="x" helper-text="hint"></cor-date-input>);
    ...
    it('wires aria-describedby to the error id when invalid + error-text present', async () => {
      const { root } = await render(<cor-date-input label="x" invalid error-text="Required"></cor-date-input>);
      const describedBy = queryNative(root)?.getAttribute('aria-describedby');
    ...
    it('uses aria-label as the accessible name when no visible label is present', async () => {
      const { root } = await render(<cor-date-input aria-label="Birthday"></cor-date-input>);
      const native = queryNative(root);
