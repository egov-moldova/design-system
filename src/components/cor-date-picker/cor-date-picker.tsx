...
import { DATE_PICKER_BREAKPOINTS, DATE_PICKER_MODES, DATE_PICKER_VIEWS } from './cor-date-picker.types';
import type {
...
export class CorDatePicker {
   ...
   */
  @Event() corChange!: EventEmitter<DatePickerChangeDetail>;

  /** Fires when the visible month changes (arrows, swipe, keyboard). `month` is 0-indexed. */
  @Event() corMonthChange!: EventEmitter<DatePickerMonthChangeDetail>;

  private readonly instanceId = ++datePickerInstanceCounter;
  private readonly gridLabelId = `cor-date-picker-grid-${this.instanceId}`;
  private readonly titleId = `cor-date-picker-title-${this.instanceId}`;
  ...
  validateMode(next: DatePickerMode) {
    if (!DATE_PICKER_MODES.includes(next)) {
      console.warn(
        `[cor-date-picker] mode="${String(next)}" is not supported. Supported: ${DATE_PICKER_MODES.join(
          ', ',
  ...
  validateBreakpoint(next: DatePickerBreakpoint) {
    if (!DATE_PICKER_BREAKPOINTS.includes(next)) {
      console.warn(
        `[cor-date-picker] breakpoint="${String(next)}" is not supported. Supported: ${DATE_PICKER_BREAKPOINTS.join(
          ', ',
  ...
  private goToMonth(delta: number) {
    ...
    this.corMonthChange.emit({ year: this.viewYear, month: this.viewMonth });
  }
  ...
  private selectDay(iso: string) {
    ...
    if (this.mode === 'range') {
      ...
      if (!this.rangeStart || (this.rangeStart && this.rangeEnd)) {
        ...
        this.value = [iso];
        this.corChange.emit({ value: [iso], rangeStart: iso, rangeEnd: undefined });
        return;
      }
      ...
      this.corChange.emit({ value: [start, end], rangeStart: start, rangeEnd: end });
      return;
    }
    if (this.mode === 'multi') {
      ...
      this.value = current;
      this.corChange.emit({ value: current });
    }
  }
