...
describe('cor-date-picker', () => {
  describe('defaults + prop reflection', () => {
    ...
    it.each(DATE_PICKER_MODES)('reflects mode="%s" to host', async mode => {
      const { root } = await render(<cor-date-picker mode={mode}></cor-date-picker>);
      expect(root?.getAttribute('mode')).toBe(mode);
    });

    it.each(DATE_PICKER_BREAKPOINTS)('reflects breakpoint="%s" to host', async breakpoint => {
      const { root } = await render(<cor-date-picker breakpoint={breakpoint}></cor-date-picker>);
  ...
  describe('grid + locale', () => {
    it('renders a 6×7 day grid (42 cells)', async () => {
      const { root } = await render(<cor-date-picker value="2026-05-23"></cor-date-picker>);
      const cells = queryCells(root);
      expect(cells.length).toBe(42);
    });

    it('renders 7 weekday labels', async () => {
      const { root } = await render(<cor-date-picker locale="ro-RO"></cor-date-picker>);
    ...
    it('switches month label to English when locale=en-US', async () => {
      const { root } = await render(<cor-date-picker value="2026-05-23" locale="en-US"></cor-date-picker>);
      const title = queryTitle(root);
    ...
    it('first day of week becomes Sunday when firstDayOfWeek=0', async () => {
      const { root } = await render(<cor-date-picker locale="en-US" firstDayOfWeek={0}></cor-date-picker>);
      const labels = queryDayLabels(root);
      const firstLabelLong = labels[0]?.getAttribute('aria-label')?.toLowerCase();
  ...
  describe('selection — single mode', () => {
    it('emits corChange with ISO value on click', async () => {
      const { root } = await render(<cor-date-picker mode="single" value="2026-05-01"></cor-date-picker>);
      const onChange = vi.fn();
      root?.addEventListener('corChange', onChange);
    ...
    it('marks the selected day with aria-selected="true"', async () => {
      const { root } = await render(<cor-date-picker mode="single" value="2026-05-15"></cor-date-picker>);
    ...
    it('does not emit when clicking a disabled date', async () => {
      const { root } = await render(
        <cor-date-picker mode="single" value="2026-05-01" min="2026-05-10" max="2026-05-20"></cor-date-picker>,
      );
      const onChange = vi.fn();
      root?.addEventListener('corChange', onChange);
  ...
  describe('selection — range mode', () => {
    it('sets rangeStart on the first click and rangeEnd on the second', async () => {
      const { root } = await render(<cor-date-picker mode="range"></cor-date-picker>);
      const onChange = vi.fn();
      root?.addEventListener('corChange', onChange);
    ...
    it('starts a new range when both endpoints are already set', async () => {
      const { root } = await render(
        <cor-date-picker mode="range" rangeStart="2026-05-10" rangeEnd="2026-05-15"></cor-date-picker>,
      );
  ...
  describe('selection — multi mode', () => {
    it('toggles individual dates into and out of the value array', async () => {
      const { root } = await render(<cor-date-picker mode="multi"></cor-date-picker>);
      const onChange = vi.fn();
      root?.addEventListener('corChange', onChange);
      queryCellByIso(root, '2026-05-10')?.click();
      await flush();
  ...
  describe('keyboard navigation', () => {
    ...
    it('PageDown advances the month', async () => {
      ...
      root?.addEventListener('corMonthChange', onMonthChange);
      const cell = queryCellByIso(root, '2026-05-15');
      cell?.focus();
      cell?.dispatchEvent(
  ...
  describe('navigation header', () => {
    it('clicking the next nav button advances one month', async () => {
      const { root } = await render(<cor-date-picker value="2026-05-15"></cor-date-picker>);
      const onMonthChange = vi.fn();
      root?.addEventListener('corMonthChange', onMonthChange);
  ...
  describe('ARIA + a11y contracts', () => {
    ...
    it('honors user-supplied aria-label when provided', async () => {
      const { root } = await render(
        <cor-date-picker value="2026-05-15" aria-label="Selectează data"></cor-date-picker>,
      );
    ...
    it('disabled dates expose aria-disabled', async () => {
      const { root } = await render(
        <cor-date-picker value="2026-05-15" disabledDates={['2026-05-09', '2026-05-10']}></cor-date-picker>,
      );
  ...
  describe('today shortcut + footer', () => {
    ...
    it('hides the Today shortcut when hideTodayShortcut is set', async () => {
      const { root } = await render(<cor-date-picker hideTodayShortcut></cor-date-picker>);
      const todayButton = root?.shadowRoot?.querySelector('button.today-button');
      expect(todayButton).toBeNull();
    });

    it('clicking Today resets the view to the current month', async () => {
      const { root } = await render(<cor-date-picker value="2020-01-15"></cor-date-picker>);
      const onMonthChange = vi.fn();
      root?.addEventListener('corMonthChange', onMonthChange);
  ...
  describe('breakpoint behavior', () => {
    it('renders a drag handle when breakpoint=mobile', async () => {
      const { root } = await render(<cor-date-picker breakpoint="mobile"></cor-date-picker>);
    ...
    it('does NOT render a drag handle when breakpoint=desktop', async () => {
      const { root } = await render(<cor-date-picker breakpoint="desktop"></cor-date-picker>);
      expect(root?.shadowRoot?.querySelector('.drag-handle')).toBeNull();
    });

    it('does NOT render a drag handle when breakpoint=docked', async () => {
      const { root } = await render(<cor-date-picker breakpoint="docked"></cor-date-picker>);
      expect(root?.shadowRoot?.querySelector('.drag-handle')).toBeNull();
    });
  });
  ...
  describe('edge cases', () => {
    it('renders Feb 29 correctly in a leap year', async () => {
      const { root } = await render(<cor-date-picker value="2024-02-29"></cor-date-picker>);
    ...
    it('does NOT render Feb 29 in a non-leap year', async () => {
      const { root } = await render(<cor-date-picker value="2026-02-15"></cor-date-picker>);
    ...
    it('Dec→Jan boundary renders without crashing', async () => {
      const { root } = await render(<cor-date-picker value="2026-12-31"></cor-date-picker>);
