...
import '../cor-modal';
import { MODAL_CLOSE_REASONS, MODAL_SIZES, MODAL_VARIANTS } from '../cor-modal.types';
import type { ModalCloseEvent, ModalCloseReason } from '../cor-modal.types';
...
// the design system (see cor-input-chip, cor-chip, cor-select-input specs) is
// to invoke the registered handler off the component instance directly. The
// arrow-function class properties on `CorModal` are visible on the host
...
describe('cor-modal', () => {
  describe('defaults + prop reflection', () => {
    it('renders with default props reflected on host', async () => {
      const { root } = await render(<cor-modal title-text="x"></cor-modal>);
    ...
    it.each(MODAL_SIZES)('reflects size="%s" to host', async size => {
      const { root } = await render(<cor-modal size={size} title-text="x"></cor-modal>);
      expect(root?.getAttribute('size')).toBe(size);
    });

    it.each(MODAL_VARIANTS)('reflects variant="%s" to host', async variant => {
      const { root } = await render(<cor-modal variant={variant} title-text="x"></cor-modal>);
      expect(root?.getAttribute('variant')).toBe(variant);
    });

    it('reflects destructive to host', async () => {
      const { root } = await render(<cor-modal destructive title-text="x"></cor-modal>);
    ...
    it('reflects closable="false" by removing the attribute', async () => {
      const { root } = await render(<cor-modal closable={false} title-text="x"></cor-modal>);
  ...
  describe('shadow structure', () => {
    it('renders an internal <dialog> with role="dialog" semantics', async () => {
      const { root } = await render(<cor-modal title-text="Confirm"></cor-modal>);
    ...
    it('wires aria-describedby to the body id', async () => {
      const { root } = await render(<cor-modal title-text="Confirm">Body copy.</cor-modal>);
      const dialog = queryDialog(root);
    ...
    it('renders the heading text via `title-text` prop', async () => {
      const { root } = await render(<cor-modal title-text="Confirmă plata"></cor-modal>);
    ...
    it('omits the close button when `closable=false`', async () => {
      const { root } = await render(<cor-modal title-text="x" closable={false}></cor-modal>);
      expect(queryCloseButton(root)).toBeNull();
    });

    it('uses the close-label prop as the close button accessible name', async () => {
      const { root } = await render(<cor-modal title-text="x" close-label="Închide dialogul"></cor-modal>);
  ...
  describe('variant axis', () => {
    it('default variant exposes a standard header bar', async () => {
      const { root } = await render(<cor-modal variant="default" title-text="x"></cor-modal>);
    ...
    it('with-image variant renders the image header container', async () => {
      const { root } = await render(<cor-modal variant="with-image" title-text="x"></cor-modal>);
    ...
    it('with-icon variant skips the top header bar and exposes the icon slot in the body', async () => {
      const { root } = await render(<cor-modal variant="with-icon" title-text="x"></cor-modal>);
    ...
    it('with-icon variant hides the floating close affordance when closable=false', async () => {
      const { root } = await render(<cor-modal variant="with-icon" title-text="x" closable={false}></cor-modal>);
    ...
    it('with-image variant drops aria-labelledby when no title is provided', async () => {
      const { root } = await render(<cor-modal variant="with-image"></cor-modal>);
      const dialog = queryDialog(root);
  ...
  describe('open/close lifecycle', () => {
    it('reflects open attribute when the prop is set', async () => {
      const { root } = await render(<cor-modal open title-text="x"></cor-modal>);
      await flush();
    ...
    it('closes via the imperative closeModal() method with default reason', async () => {
      ...
      const { root } = await render(
        <cor-modal
          open
          title-text="x"
          onCorClose={(e: CustomEvent<ModalCloseEvent>) => events.push(e.detail)}
        ></cor-modal>,
      );
      await flush();
    ...
    it('closeModal() is a no-op when already closed', async () => {
      const events: ModalCloseEvent[] = [];
      const { root } = await render(
        <cor-modal title-text="x" onCorClose={(e: CustomEvent<ModalCloseEvent>) => events.push(e.detail)}></cor-modal>,
      );
      await flush();
  ...
  describe('event emissions', () => {
    it('emits corOpen when the dialog becomes visible', async () => {
      const onOpen = vi.fn();
      const { root } = await render(<cor-modal title-text="x" onCorOpen={onOpen}></cor-modal>);
      await (root as unknown as { openModal: () => Promise<void> }).openModal();
      await flush();
      expect(onOpen).toHaveBeenCalledTimes(1);
    });

    it('emits corClose with reason="close-button" when the × is clicked', async () => {
      const onClose = vi.fn();
      const { root } = await render(<cor-modal open title-text="x" onCorClose={onClose}></cor-modal>);
      await flush();
      const closeBtn = queryCloseButton(root)!;
      closeBtn.click();
      await flush();
    ...
    it('emits corClose with reason="close-button" when Enter is pressed on the ×', async () => {
      const onClose = vi.fn();
      const { root } = await render(<cor-modal open title-text="x" onCorClose={onClose}></cor-modal>);
      await flush();
    ...
    it('emits corClose with reason="close-button" when Space is pressed on the ×', async () => {
      const onClose = vi.fn();
      const { root } = await render(<cor-modal open title-text="x" onCorClose={onClose}></cor-modal>);
      await flush();
    ...
    it('emits corClose with reason="escape" when the dialog cancel event fires and closeOnEscape=true', async () => {
      const onClose = vi.fn();
      const { root } = await render(<cor-modal open title-text="x" onCorClose={onClose}></cor-modal>);
      await flush();
  ...
  describe('closeOnBackdrop behavior', () => {
    it('emits corClose with reason="backdrop" when the dialog itself is clicked outside the surface', async () => {
      const onClose = vi.fn();
      const { root } = await render(<cor-modal open title-text="x" onCorClose={onClose}></cor-modal>);
      await flush();
      const dialog = queryDialog(root)!;
    ...
    it('does NOT dismiss when the click target is a descendant of the dialog (e.g. body content)', async () => {
      const onClose = vi.fn();
      const { root } = await render(
        <cor-modal open title-text="x" onCorClose={onClose}>
          Inside content
        </cor-modal>,
      );
      await flush();
  ...
  describe('open prop watcher', () => {
    ...
    it('emits corOpen exactly once per true transition', async () => {
      const onOpen = vi.fn();
      const { root } = await render(<cor-modal title-text="x" onCorOpen={onOpen}></cor-modal>);
