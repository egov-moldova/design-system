...
import { NOTIFICATION_ASSERTIVE_VARIANTS, NOTIFICATION_DEFAULT_ICONS } from './cor-notification.types';
 ...
 *                    default icon.
 * @slot actions - Optional inline action group (typically `cor-button` or
 *                 `cor-link`). Aligned to the trailing edge before the close
 *                 button when present.
 */
@Component({
  tag: 'cor-notification',
  styleUrl: 'cor-notification.css',
  shadow: true,
})
export class CorNotification {
  /**
   ...
   * `corClose`; the consumer is responsible for removing the notification
   * from the DOM.
   * @default false
   */
  ...
  /**
   * Override the default `cor-icon` name for the variant (e.g. swap
  ...
  @Element() host!: HTMLCorNotificationElement;
