...
| `closable`          | `closable`           | When `true`, renders a trailing close button. Activating it emits `corClose`; the consumer is responsible for removing the notification from the DOM.                                                                                                                                 | `boolean`                                                    | `false`     |
...
| `iconName`          | `icon-name`          | Override the default `cor-icon` name for the variant (e.g. swap `circle-info-filled` for a custom glyph). When the `icon-start` slot is populated, this prop is ignored.                                                                                                              | `string \| undefined`                                        | `undefined` |
...
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| `corClose` | Fires when the user activates the close button. Payload is `void` — the consumer is responsible for the dismiss animation / DOM removal. | `CustomEvent<void>` |
...
| `"actions"`    | Optional inline action group (typically `cor-button` or           `cor-link`). Aligned to the trailing edge before the close           button when present. |
...
graph TD;
  ...
  style cor-notification fill:#f9f,stroke:#333,stroke-width:4px
```
