...
describe('cor-notification', () => {
  describe('defaults', () => {
    it('renders with default props reflected on host', async () => {
      const { root } = await render(<cor-notification>Mesaj</cor-notification>);
  ...
  describe('variant prop', () => {
    it.each(NOTIFICATION_VARIANTS)('reflects variant="%s" on the host', async variant => {
      const { root } = await render(<cor-notification variant={variant}>Mesaj</cor-notification>);
  ...
  describe('notificationStyle prop', () => {
    it.each(NOTIFICATION_STYLES)('reflects notification-style="%s" on the host', async style => {
      const { root } = await render(<cor-notification notification-style={style}>Mesaj</cor-notification>);
  ...
  describe('closable behaviour', () => {
    it('renders a close button when closable is set', async () => {
      const { root } = await render(<cor-notification closable>Mesaj</cor-notification>);
    ...
    it('honors a custom close-label prop', async () => {
      const { root } = await render(
        <cor-notification closable close-label="Dismiss">
          Message
        </cor-notification>,
      );
    ...
    it('emits corClose on click', async () => {
      const handler = vi.fn();
      const { root } = await render(
        <cor-notification closable onCorClose={handler}>
          Mesaj
        </cor-notification>,
      );
    ...
    it('emits corClose on Enter keypress', async () => {
      const handler = vi.fn();
      const { root } = await render(
        <cor-notification closable onCorClose={handler}>
          Mesaj
    ...
    it('emits corClose on Space keypress', async () => {
      const handler = vi.fn();
      const { root } = await render(
        <cor-notification closable onCorClose={handler}>
          Mesaj
    ...
    it('does not emit corClose on unrelated key presses', async () => {
      const handler = vi.fn();
      const { root } = await render(
        <cor-notification closable onCorClose={handler}>
          Mesaj
  ...
  describe('titleText prop', () => {
    it('renders the title element when title-text is provided', async () => {
      const { root } = await render(
        <cor-notification title-text="Plată reușită">Tranzacția a fost confirmată.</cor-notification>,
      );
    ...
    it('adds has-title class to the host when title-text is provided', async () => {
      const { root } = await render(<cor-notification title-text="Plată reușită">Mesaj</cor-notification>);
      expect(root?.classList.contains('has-title')).toBe(true);
    });

    it('does not add has-title class when title-text is empty string', async () => {
      const { root } = await render(<cor-notification title-text="">Mesaj</cor-notification>);
      expect(root?.classList.contains('has-title')).toBe(false);
    });
  });

  describe('iconName prop', () => {
    it('forwards iconName to the default cor-icon when slot is empty', async () => {
      const { root } = await render(<cor-notification icon-name="receipt-check-filled">Bon fiscal</cor-notification>);
    ...
    it('falls back to the per-variant default icon when iconName is unset', async () => {
      const { root } = await render(<cor-notification variant="danger">Eroare</cor-notification>);
      const icon = root?.shadowRoot?.querySelector('cor-icon');
      expect(icon?.getAttribute('name')).toBe('circle-error-filled');
    });

    it('uses circle-checkmark-filled for positive variant', async () => {
      const { root } = await render(<cor-notification variant="positive">OK</cor-notification>);
      const icon = root?.shadowRoot?.querySelector('cor-icon');
      expect(icon?.getAttribute('name')).toBe('circle-checkmark-filled');
    });

    it('uses warning-filled for warning variant', async () => {
      const { root } = await render(<cor-notification variant="warning">Atenție</cor-notification>);
      const icon = root?.shadowRoot?.querySelector('cor-icon');
  ...
  describe('label rendering', () => {
    it('renders default-slot text content', async () => {
      const { root } = await render(<cor-notification>Mesaj important</cor-notification>);
      expect((root?.textContent ?? '').trim()).toContain('Mesaj important');
    });

    it('supports Romanian diacritics in the body', async () => {
      const { root } = await render(<cor-notification>Înălțime mărită — așteaptă confirmarea</cor-notification>);
      expect((root?.textContent ?? '').trim()).toContain('Înălțime mărită');
    });

    it('supports Romanian diacritics in the title', async () => {
      const { root } = await render(
        <cor-notification title-text="Sesiunea a expirat">Reconectați-vă</cor-notification>,
      );
