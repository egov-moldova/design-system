 ...
 */
@Component({
  tag: 'cor-numeric-input',
  styleUrl: 'cor-numeric-input.css',
...
export class CorNumericInput {
  ...
  private readonly labelId = `cor-numeric-input-label-${this.instanceId}`;
  private readonly helperId = `cor-numeric-input-helper-${this.instanceId}`;
  private readonly errorId = `cor-numeric-input-error-${this.instanceId}`;
  ...
  validateVariant(next: NumericInputVariant) {
    if (!NUMERIC_INPUT_VARIANTS.includes(next)) {
      console.warn(
        `[cor-numeric-input] variant="${String(next)}" is not supported. Supported: ${NUMERIC_INPUT_VARIANTS.join(
          ', ',
  ...
  validateSize(next: NumericInputSize) {
    if (!NUMERIC_INPUT_SIZES.includes(next)) {
      console.warn(
        `[cor-numeric-input] size="${String(next)}" is not supported. Supported: ${NUMERIC_INPUT_SIZES.join(
          ', ',
        )}. Falling back to "md".`,
      );
      this.size = 'md';
  ...
  private performStep(direction: NumericInputStepDirection): void {
    ...
    this.value = next;
    this.displayValue = this.formatForDisplay(next);
    this.corStep.emit({ direction, value: next });
    this.corInput.emit({ value: next });
  ...
  private handleInput = (ev: Event) => {
    ...
    if (this.min !== undefined && parsed < this.min) {
      this.corError.emit({ reason: 'out-of-range', rawValue: raw });
    } else if (this.max !== undefined && parsed > this.max) {
      this.corError.emit({ reason: 'out-of-range', rawValue: raw });
    }
    this.value = parsed;
    this.corInput.emit({ value: parsed });
  };
  ...
  private commitFromDisplay(): void {
    ...
    this.corChange.emit({ value: committed });
  }
      ...
      <Host class={hostClasses} aria-busy={this.loading ? 'true' : null}>
        ...
        {this.hasErrorMessage() ? (
          <div class="assistive assistive-error" id={this.errorId} part="error">
            <cor-icon class="assistive-icon" name="circle-error-filled" size={iconSize} color="icon-danger-default" />
