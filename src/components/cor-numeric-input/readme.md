...
| `loading`        | `loading`         | Loading state. When true the control becomes uninteractive and a brand `cor-spinner` replaces the trailing stepper stack. The host carries `aria-busy="true"` for assistive technologies.                                                                                                 | `boolean`                                 | `false`     |
...
| `"icon-start"` | Leading slot rendered before the value. Accepts a `cor-icon` (icon-leading variant) OR a plain currency/unit text symbol (prefix variant, e.g. `€`, `$`, `MDL`) — Figma master treats these as the same slot. |
