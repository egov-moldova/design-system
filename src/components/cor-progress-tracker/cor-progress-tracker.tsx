...
} from './cor-progress-tracker.types';
 ...
 */
@Component({
  tag: 'cor-progress-tracker',
  styleUrl: 'cor-progress-tracker.css',
  shadow: true,
})
export class CorProgressTracker {
  /**
   * Layout orientation.
   ...
   * `corStepClick`. Pending and error steps remain non-actionable in this mode.
   * @default false
   */
   ...
   */
  @Event({ bubbles: true, composed: true }) corStepClick!: EventEmitter<ProgressTrackerStepClickDetail>;
  ...
  private readonly handleStepClick = (ev: MouseEvent, step: ProgressTrackerStep, index: number) => {
    ...
    const dispatched = this.corStepClick.emit({ index, step });
  ...
  private readonly handleKeyDown = (ev: KeyboardEvent, step: ProgressTrackerStep, index: number) => {
    ...
    ev.preventDefault();
    this.corStepClick.emit({ index, step });
  };
