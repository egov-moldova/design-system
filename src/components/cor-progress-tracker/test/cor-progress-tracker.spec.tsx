...
describe('cor-progress-tracker', () => {
  let warnSpy: ReturnType<typeof vi.spyOn>;
  let fetchSpy: ReturnType<typeof vi.spyOn>;

  beforeEach(() => {
    // cor-icon resolves assets via `getAssetPath()` which requires a base URL.
    setAssetPath('http://localhost/');
    warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    // cor-icon lazy-fetches the SVG sprite; stub it so tests stay deterministic.
  ...
  describe('default rendering', () => {
    it('renders the list landmark with a default aria-label', async () => {
      const { root } = await render(<cor-progress-tracker steps={ROMANIAN_STEPS}></cor-progress-tracker>);
    ...
    it('honors a custom aria-label (Romanian)', async () => {
      const { root } = await render(
        <cor-progress-tracker steps={ROMANIAN_STEPS} aria-label="Pași"></cor-progress-tracker>,
      );
    ...
    it('reflects orientation on the host', async () => {
      const { root } = await render(
        <cor-progress-tracker steps={ROMANIAN_STEPS} orientation="vertical"></cor-progress-tracker>,
      );
      expect(root?.getAttribute('orientation')).toBe('vertical');
    });

    it('reflects interactive on the host when true', async () => {
      const { root } = await render(<cor-progress-tracker steps={ROMANIAN_STEPS} interactive></cor-progress-tracker>);
    ...
    it('renders the supporting text line when provided', async () => {
      ...
      const { root } = await render(<cor-progress-tracker steps={stepsWithSupport}></cor-progress-tracker>);
  ...
  describe('status mirroring to ARIA + class', () => {
    it('applies step--{status} modifier classes', async () => {
      const { root } = await render(<cor-progress-tracker steps={ALL_STATES}></cor-progress-tracker>);
      const items = querySteps(root);
    ...
    it('renders a checkmark icon for completed steps', async () => {
      ...
      const icon = firstStep.querySelector('cor-icon');
      // cor-icon's `name` prop is non-reflecting — read via property, not attribute.
    ...
    it('renders a cross icon for error steps', async () => {
      ...
      const icon = errorStep.querySelector('cor-icon');
    ...
    it('honours a custom iconName override on the step', async () => {
      ...
      const { root } = await render(<cor-progress-tracker steps={stepsWithIcon}></cor-progress-tracker>);
      const icon = querySteps(root)[0].querySelector('cor-icon');
      // cor-icon's `name` prop is non-reflecting — read via property, not attribute.
  ...
  describe('currentStep override', () => {
    it('promotes the step at currentStep index to current', async () => {
      ...
      const { root } = await render(<cor-progress-tracker steps={allPending} currentStep={1}></cor-progress-tracker>);
      const items = querySteps(root);
      expect(items[1].classList.contains('step--current')).toBe(true);
    ...
    it('does NOT overwrite an error status', async () => {
      ...
      const { root } = await render(<cor-progress-tracker steps={steps} currentStep={1}></cor-progress-tracker>);
      const items = querySteps(root);
  ...
  describe('interactive mode', () => {
    ...
    it('emits corStepClick with { index, step } when a step is clicked', async () => {
      const onClick = vi.fn();
      const { root } = await render(
        <cor-progress-tracker steps={ROMANIAN_STEPS} interactive onCorStepClick={onClick}></cor-progress-tracker>,
      );
      const buttons = queryTriggers(root);
    ...
    it('does NOT emit corStepClick when interactive is false', async () => {
      const onClick = vi.fn();
      const { root } = await render(
        <cor-progress-tracker steps={ROMANIAN_STEPS} onCorStepClick={onClick}></cor-progress-tracker>,
      );
    ...
    it('does NOT emit corStepClick for pending steps even in interactive mode', async () => {
      const onClick = vi.fn();
      ...
      const { root } = await render(
        <cor-progress-tracker steps={stepsWithPending} interactive onCorStepClick={onClick}></cor-progress-tracker>,
      );
    ...
    it('emits corStepClick via the Enter key on a focusable trigger', async () => {
      const onClick = vi.fn();
      const { root } = await render(
        <cor-progress-tracker steps={ROMANIAN_STEPS} interactive onCorStepClick={onClick}></cor-progress-tracker>,
      );
      // JSX-bound onKeyDown handlers are class methods; invoke directly per the
      // Stencil vitest convention (see cor-chip spec for the same pattern).
      type Instance = {
    ...
    it('emits corStepClick via the Space key on a focusable trigger', async () => {
      const onClick = vi.fn();
      const { root } = await render(
        <cor-progress-tracker steps={ROMANIAN_STEPS} interactive onCorStepClick={onClick}></cor-progress-tracker>,
      );
    ...
    it('honours per-step disabled flag even when status would be actionable', async () => {
      ...
      const { root } = await render(<cor-progress-tracker steps={steps} interactive></cor-progress-tracker>);
      const buttons = queryTriggers(root);
  ...
  describe('accessible names', () => {
    ...
    it('includes supportingText in the accessible name when provided', async () => {
      ...
      const { root } = await render(<cor-progress-tracker steps={steps}></cor-progress-tracker>);
  ...
  describe('edge cases', () => {
    it('renders an empty list when steps is undefined', async () => {
      const { root } = await render(<cor-progress-tracker></cor-progress-tracker>);
    ...
    it('renders no connector for a single-step tracker', async () => {
      const { root } = await render(
        <cor-progress-tracker steps={[{ label: 'Singur', status: 'current' }]}></cor-progress-tracker>,
      );
