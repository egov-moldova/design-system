/**
 * Pure helpers used by `cor-receipt`. Extracted so they can be unit-tested
 * without rendering the component.
 */

import type { LogoName } from '../cor-logo/cor-logo.types';
import type { TagSemantic } from '../cor-tag/cor-tag.types';
...
/**
 * Mapping from service → cor-logo asset name (`with-name` layout).
...
/**
 * Mapping from status → cor-tag semantic role.
 * cor-tag's semantic vocabulary uses `success` (not `positive`); the
