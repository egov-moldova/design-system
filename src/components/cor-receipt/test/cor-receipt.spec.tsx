...
describe('cor-receipt — pure helpers', () => {
  describe('maskIdnp', () => {
...
const queryStatus = (root: Element | null | undefined): Element | null =>
  root?.shadowRoot?.querySelector('cor-tag.receipt-status') ?? null;
...
const queryButtons = (root: Element | null | undefined): NodeListOf<Element> | undefined =>
  root?.shadowRoot?.querySelectorAll('.receipt-actions cor-button');

describe('cor-receipt — component', () => {
  describe('defaults', () => {
    it('reflects service="mpay" by default', async () => {
      const { root } = await render(<cor-receipt></cor-receipt>);
    ...
    it('renders one default cor-button per primary action', async () => {
      const { root } = await render(<cor-receipt status="paid" amount="150,00"></cor-receipt>);
  ...
  describe('service prop', () => {
    it.each(RECEIPT_SERVICES)('reflects service="%s" on the host', async service => {
      const { root } = await render(<cor-receipt service={service}></cor-receipt>);
      expect(root?.getAttribute('service')).toBe(service);
    });
    it('falls back to mpay on an invalid service string', async () => {
      const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
      // @ts-expect-error — exercising runtime guard
      const { root } = await render(<cor-receipt service="bogus"></cor-receipt>);
      expect(root?.getAttribute('service')).toBe('mpay');
  ...
  describe('status prop', () => {
    it.each(RECEIPT_STATUSES)('renders the matching Romanian tag label for "%s"', async status => {
      const { root } = await render(<cor-receipt status={status}></cor-receipt>);
    ...
    it('includes status in the resolved aria-label', async () => {
      const { root } = await render(<cor-receipt status="paid"></cor-receipt>);
  ...
  describe('amount block', () => {
    it('renders the amount and currency when amount is set', async () => {
      const { root } = await render(<cor-receipt amount="150,00" currency="MDL"></cor-receipt>);
  ...
  describe('QR rendering', () => {
    it('renders the inline SVG QR when qrData is provided', async () => {
      const { root } = await render(<cor-receipt qr-data="https://verify.mpay.gov.md/tx/0001"></cor-receipt>);
    ...
    it('the QR figure carries a Romanian accessible label by default', async () => {
      const { root } = await render(<cor-receipt qr-data="x"></cor-receipt>);
  ...
  describe('action events', () => {
    it('emits corDownload with the action detail when the primary button is activated', async () => {
      const { root } = await render(<cor-receipt service="mpay" status="paid" transaction-id="TXN-1"></cor-receipt>);
      const handler = vi.fn();
      root?.addEventListener('corDownload', handler as EventListener);
      const buttons = queryButtons(root);
      // First button is "Descarcă PDF" — cor-button is Pattern B (renders its own
    ...
    it('emits corEmail, corPrint, corShare on the remaining buttons', async () => {
      const { root } = await render(<cor-receipt status="paid"></cor-receipt>);
      const emailFn = vi.fn();
      const printFn = vi.fn();
      const shareFn = vi.fn();
      ...
      window.print = vi.fn();
      root?.addEventListener('corEmail', emailFn as EventListener);
      root?.addEventListener('corPrint', printFn as EventListener);
      root?.addEventListener('corShare', shareFn as EventListener);
      const buttons = queryButtons(root);
  ...
  describe('locale formatting', () => {
    it('formats the date through the supplied locale', async () => {
      const { root } = await render(<cor-receipt date="2026-05-22T14:32:00Z" locale="ro-RO"></cor-receipt>);
      const text = root?.shadowRoot?.textContent ?? '';
      expect(text.toLowerCase()).toContain('mai');
    });
    it('respects an English locale override', async () => {
      const { root } = await render(<cor-receipt date="2026-05-22T14:32:00Z" locale="en-US"></cor-receipt>);
      const text = root?.shadowRoot?.textContent ?? '';
  ...
  describe('a11y contract', () => {
    it('uses an overridden aria-label when provided', async () => {
      const { root } = await render(<cor-receipt aria-label="Custom name"></cor-receipt>);
