...
} from './cor-search-input-circular.types';
 ...
 */
@Component({
  tag: 'cor-search-input-circular',
  styleUrl: 'cor-search-input-circular.css',
  shadow: { delegatesFocus: true },
  formAssociated: true,
})
export class CorSearchInputCircular {
  /**
  ...
  @Event() corInput!: EventEmitter<SearchInputCircularChangeDetail>;

  /** Fires when the value is committed (typically on `blur`). `detail.value` is the committed value. */
  @Event() corChange!: EventEmitter<SearchInputCircularChangeDetail>;

  /** Fires when the user submits the query (Enter key). `detail.value` is the submitted query. */
  @Event() corSearch!: EventEmitter<SearchInputCircularSearchDetail>;
  ...
  private readonly labelId = `cor-search-input-circular-label-${this.instanceId}`;
  private readonly helperId = `cor-search-input-circular-helper-${this.instanceId}`;
  private readonly errorId = `cor-search-input-circular-error-${this.instanceId}`;
  ...
  validateVariant(next: SearchInputCircularVariant) {
    if (!SEARCH_INPUT_CIRCULAR_VARIANTS.includes(next)) {
      console.warn(
        `[cor-search-input-circular] variant="${String(
          next,
  ...
  validateSize(next: SearchInputCircularSize) {
    if (!SEARCH_INPUT_CIRCULAR_SIZES.includes(next)) {
      console.warn(
        `[cor-search-input-circular] size="${String(
          next,
