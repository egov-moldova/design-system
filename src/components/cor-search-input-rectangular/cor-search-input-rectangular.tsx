 ...
 * @slot icon-start - Leading icon override. Defaults to `cor-icon[name="search"]`.
 * @slot icon-end - Trailing slot. Suppresses the built-in clear `×` button when content is assigned here.
 */
@Component({
  tag: 'cor-search-input-rectangular',
  styleUrl: 'cor-search-input-rectangular.css',
  shadow: { delegatesFocus: true },
  formAssociated: true,
})
export class CorSearchInputRectangular {
  /**
  ...
  private readonly labelId = `cor-search-input-rectangular-label-${this.instanceId}`;
  private readonly helperId = `cor-search-input-rectangular-helper-${this.instanceId}`;
  private readonly errorId = `cor-search-input-rectangular-error-${this.instanceId}`;
  ...
  validateVariant(next: SearchInputRectangularVariant) {
    if (!SEARCH_INPUT_RECTANGULAR_VARIANTS.includes(next)) {
      console.warn(
        `[cor-search-input-rectangular] variant="${String(
          next,
  ...
  validateSize(next: SearchInputRectangularSize) {
    if (!SEARCH_INPUT_RECTANGULAR_SIZES.includes(next)) {
      console.warn(
        `[cor-search-input-rectangular] size="${String(
          next,
