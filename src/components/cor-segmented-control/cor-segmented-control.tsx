...
} from './cor-segmented-control.types';
 ...
 */
@Component({
  tag: 'cor-segmented-control',
  styleUrl: 'cor-segmented-control.css',
  shadow: { delegatesFocus: true },
  formAssociated: true,
})
export class CorSegmentedControl {
  /**
  ...
  @Element() host!: HTMLCorSegmentedControlElement;

  @AttachInternals() internals!: ElementInternals;

  /** Fires when the selected segment changes. `detail.value` is the new selection. */
  @Event() corChange!: EventEmitter<SegmentedControlChangeDetail>;

  private readonly instanceId = ++segmentedControlInstanceCounter;
  private readonly groupId = `cor-segmented-control-${this.instanceId}`;
  ...
  validateSize(next: SegmentedControlSize) {
    if (!SEGMENTED_CONTROL_SIZES.includes(next)) {
      console.warn(
        `[cor-segmented-control] size="${String(next)}" is not supported. Supported: ${SEGMENTED_CONTROL_SIZES.join(
          ', ',
        )}. Falling back to "md".`,
      );
      this.size = 'md';
  ...
  private selectSegment(segment: SegmentedControlSegment) {
    ...
    this.corChange.emit({ value: segment.value });
  }
        ...
        <div class="track" part="track" id={this.groupId}>
          {segments.map((segment, index) => {
            ...
            return (
              ...
              >
                {segment.iconName ? (
                  <cor-icon class="segment__icon" name={segment.iconName} size={20}></cor-icon>
                ) : null}
