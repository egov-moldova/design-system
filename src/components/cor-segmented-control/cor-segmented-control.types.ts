...
/**
 * Declarative description of a single segment within a `cor-segmented-control`.
 *
 * The host renders one segment per entry of the `segments` prop. Each segment
 * exposes a click + keyboard target that emits `corChange` when activated.
 */
export interface SegmentedControlSegment {
   ...
   * @see src/components/cor-icon/cor-icon.providers.ts
   */
  iconName?: string;
}

/**
 * `detail` payload of the `corChange` event.
 *
