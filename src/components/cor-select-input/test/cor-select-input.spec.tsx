...
import '../cor-select-input';

import { SELECT_INPUT_SIZES, SELECT_INPUT_VARIANTS } from '../cor-select-input.types';
import type { SelectOption } from '../cor-select-input.types';
...
describe('cor-select-input', () => {
  describe('defaults + prop reflection', () => {
    it('renders with default props reflected on host', async () => {
      const { root } = await render(<cor-select-input label="Country"></cor-select-input>);
    ...
    it.each(SELECT_INPUT_VARIANTS)('reflects variant="%s" to host', async variant => {
      const { root } = await render(<cor-select-input variant={variant} label="x"></cor-select-input>);
      expect(root?.getAttribute('variant')).toBe(variant);
    });

    it.each(SELECT_INPUT_SIZES)('reflects size="%s" to host', async size => {
      const { root } = await render(<cor-select-input size={size} label="x"></cor-select-input>);
  ...
  describe('shadow structure', () => {
    ...
    it('renders the label text via `label` prop', async () => {
      const { root } = await render(<cor-select-input label="Country code"></cor-select-input>);
    ...
    it('shows the placeholder when no value is selected', async () => {
      const { root } = await render(<cor-select-input label="x" placeholder="Pick one"></cor-select-input>);
    ...
    it('shows the selected option label when value is set', async () => {
      const { root } = await render(<cor-select-input label="x" value="opt-2"></cor-select-input>);
      await setOptions(root, baseOptions);
      const trigger = queryTrigger(root);
    ...
    it('renders a helper assistive row when `helper-text` is set', async () => {
      const { root } = await render(<cor-select-input label="x" helper-text="Helpful tip"></cor-select-input>);
    ...
    it('renders an error assistive row with the error icon when invalid + error-text', async () => {
      const { root } = await render(<cor-select-input label="x" invalid error-text="Required"></cor-select-input>);
      const assistive = queryAssistive(root);
    ...
    it('error message takes priority over helper text', async () => {
      const { root } = await render(
        <cor-select-input label="x" invalid helper-text="Hint" error-text="Required"></cor-select-input>,
      );
      const assistive = queryAssistive(root);
      expect(assistive?.textContent).toContain('Required');
  ...
  describe('listbox behaviour', () => {
    ...
    it('emits corOpen / corClose when toggling', async () => {
      const onOpen = vi.fn();
      const onClose = vi.fn();
      const { root } = await render(
        <cor-select-input label="x" onCorOpen={onOpen} onCorClose={onClose}></cor-select-input>,
      );
      await setOptions(root, baseOptions);
      const trigger = queryTrigger(root)!;
      trigger.click();
    ...
    it('renders one role="option" per resolved option', async () => {
      const { root } = await render(<cor-select-input label="x" open></cor-select-input>);
      await setOptions(root, baseOptions);
    ...
    it('marks the matching option as selected', async () => {
      const { root } = await render(<cor-select-input label="x" value="opt-2" open></cor-select-input>);
      await setOptions(root, baseOptions);
    ...
    it('selects an option on click and emits corChange', async () => {
      const onChange = vi.fn();
      const { root } = await render(<cor-select-input label="x" open onCorChange={onChange}></cor-select-input>);
      await setOptions(root, baseOptions);
  ...
  describe('keyboard contract', () => {
    ...
    it('ArrowUp / ArrowDown skip disabled options', async () => {
      const onChange = vi.fn();
      const { root } = await render(
        <cor-select-input label="x" open value="opt-2" onCorChange={onChange}></cor-select-input>,
      );
      await setOptions(root, baseOptions);
  ...
  describe('disabled + readonly behaviour', () => {
    ...
    it('does not open on click when readonly', async () => {
      const { root } = await render(<cor-select-input label="x" readonly></cor-select-input>);
      await setOptions(root, baseOptions);
      queryTrigger(root)?.click();
      await flush();
      expect(root?.classList.contains('is-open')).toBe(false);
  ...
  describe('ARIA contract', () => {
    ...
    it('exposes aria-invalid when invalid', async () => {
      const { root } = await render(<cor-select-input label="x" invalid></cor-select-input>);
    ...
    it('wires aria-describedby to the helper id when helper-text present', async () => {
      const { root } = await render(<cor-select-input label="x" helper-text="hint"></cor-select-input>);
  ...
  describe('slots', () => {
    it('forwards content into the icon-start slot', async () => {
      const { root } = await render(
        <cor-select-input label="Country">
