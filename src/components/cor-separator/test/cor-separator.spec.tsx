...
describe('cor-separator', () => {
  it('renders with default props and the WCAG separator role', async () => {
    const { root } = await render(<cor-separator />);
  ...
  it('exposes aria-label when provided', async () => {
    const { root } = await render(<cor-separator aria-label="Section break" />);
  ...
  it('switches to the labeled layout when default slot has non-empty text', async () => {
    const { root } = await render(<cor-separator>continuă</cor-separator>);
  ...
  it('stays in the plain layout when default slot contains only whitespace', async () => {
    const { root } = await render(<cor-separator>{'   '}</cor-separator>);
  ...
  it('constructs without registering a host when registerHost=false', () => {
    const Ctor = customElements.get('cor-separator') as unknown as new (registerHost: boolean) => unknown;
