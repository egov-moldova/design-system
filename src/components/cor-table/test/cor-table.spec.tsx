...
import '../cor-table';
import '../../cor-checkbox/cor-checkbox';
...
// unupgraded `<cor-icon>` placeholder, which is sufficient for assertions
// about table behaviour, aria-sort, and selection.

import { TABLE_HEADER_STYLES, TABLE_ROW_STYLES } from '../cor-table.types';
import type { TableColumn, TableRowData } from '../cor-table.types';
...
describe('cor-table', () => {
  describe('defaults', () => {
    it('reflects default props on the host', async () => {
      const { root } = await render(<cor-table />);
  ...
  describe('rendering', () => {
    ...
    it('reflects ariaLabel onto the <table>', async () => {
      const { root, waitForChanges } = await render(<cor-table aria-label="Tabel principal" />);
      setProps(root, { columns, rows });
      await waitForChanges();
    ...
    it('adds prepended selection column when selectable', async () => {
      ...
      expect(firstHeader?.querySelector('cor-checkbox')).toBeTruthy();
    });
  });
  ...
  describe('sort interaction', () => {
    it('emits corSort with asc on first activation of a sortable header', async () => {
      const { root, waitForChanges } = await render(<cor-table />);
      setProps(root, { columns, rows });
      await waitForChanges();
      const handler = vi.fn();
      root?.addEventListener('corSort', handler as EventListener);
      const headers = queryHeaderCells(root);
      headers[0]?.click();
      await waitForChanges();
    ...
    it('does not emit corSort on non-sortable headers', async () => {
      const { root, waitForChanges } = await render(<cor-table />);
      setProps(root, { columns, rows });
      await waitForChanges();
      const handler = vi.fn();
  ...
  describe('selection', () => {
    it('emits corSelectionChange with the row id on row checkbox toggle', async () => {
      const { root, waitForChanges } = await render(<cor-table />);
      setProps(root, { columns, rows, selectable: true, selectedRows: [] });
      await waitForChanges();
      const handler = vi.fn();
      root?.addEventListener('corSelectionChange', handler as EventListener);
      const checkboxes = root?.shadowRoot?.querySelectorAll('tbody cor-checkbox');
      const firstCheckbox = checkboxes?.[0] as HTMLInputElement & { checked: boolean };
      // Simulate the corChange event from the inner checkbox.
      firstCheckbox.checked = true;
      firstCheckbox.dispatchEvent(new CustomEvent('corChange', { detail: { checked: true } }));
      await waitForChanges();
      expect(handler).toHaveBeenCalledTimes(1);
      expect((handler.mock.calls[0][0] as CustomEvent).detail.selectedRows).toEqual(['1']);
    });

    it('emits corSelectionChange with all ids when the master checkbox toggles on', async () => {
      const { root, waitForChanges } = await render(<cor-table />);
      setProps(root, { columns, rows, selectable: true, selectedRows: [] });
      await waitForChanges();
      const handler = vi.fn();
      root?.addEventListener('corSelectionChange', handler as EventListener);
      const masterCheckbox = root?.shadowRoot?.querySelector('thead cor-checkbox') as HTMLInputElement & {
        checked: boolean;
      };
      masterCheckbox.checked = true;
      masterCheckbox.dispatchEvent(new CustomEvent('corChange', { detail: { checked: true } }));
      await waitForChanges();
      expect(handler).toHaveBeenCalledTimes(1);
    ...
    it('emits empty array when master checkbox toggles off', async () => {
      ...
      masterCheckbox.dispatchEvent(new CustomEvent('corChange', { detail: { checked: false } }));
      await waitForChanges();
  ...
  describe('row click', () => {
    it('emits corRowClick with row + index on body row click', async () => {
      const { root, waitForChanges } = await render(<cor-table />);
      setProps(root, { columns, rows });
      await waitForChanges();
      const handler = vi.fn();
      root?.addEventListener('corRowClick', handler as EventListener);
      const bodyRows = queryBodyRows(root);
      bodyRows[1]?.click();
      await waitForChanges();
      expect(handler).toHaveBeenCalledTimes(1);
