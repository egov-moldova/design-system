...
/**
 * Declarative description of a single tab in a `cor-tabs` molecule.
 *
 * Each entry of `tabs` renders one tab item. As an alternative, consumers may
 * slot `<cor-tab>` children directly — the host then derives its model from
 * the slotted DOM rather than from the `tabs` prop.
 */
export interface TabDescriptor {
  /** Identity of the tab. Reflected as `value` on the underlying `<cor-tab>`. */
  value: string;
  /** Plain-text label rendered inside the tab. */
  label: string;
  /** Optional leading icon — maps to a registered `cor-icon` name. */
  iconName?: string;
  /**
...
/**
 * `detail` payload of the `corChange` event emitted by `cor-tabs` when the
