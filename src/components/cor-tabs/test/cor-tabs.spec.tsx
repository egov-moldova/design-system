...
import type { TabDescriptor } from '../cor-tabs.types';
...
const queryTabs = (root: Element | null | undefined): HTMLElement[] => {
  ...
  const light = Array.from(root?.children ?? []).filter(el => el.tagName === 'COR-TAB');
  const shadow = Array.from(root?.shadowRoot?.querySelectorAll('cor-tab') ?? []);
...
describe('cor-tabs', () => {
  describe('defaults + prop reflection', () => {
    it('renders with default props reflected on the host', async () => {
      const { root } = await render(<cor-tabs aria-label="Navigare"></cor-tabs>);
    ...
    it.each(TABS_SIZES)('reflects size="%s" to the host', async size => {
      const { root } = await render(<cor-tabs size={size} aria-label="Navigare"></cor-tabs>);
    ...
    it('renders one <cor-tab> per descriptor', async () => {
      const { root } = await render(<cor-tabs aria-label="Navigare"></cor-tabs>);
    ...
    it('honors an initial value', async () => {
      const { root } = await render(<cor-tabs aria-label="Navigare" value="notificari"></cor-tabs>);
      await setTabs(root, sampleTabs);
      const tabs = queryTabs(root);
  ...
  describe('selection + corChange', () => {
    it('emits corChange when an unselected tab is clicked', async () => {
      const onChange = vi.fn();
      const { root } = await render(<cor-tabs aria-label="Navigare" value="profil" onCorChange={onChange}></cor-tabs>);
      await setTabs(root, sampleTabs);
    ...
    it('does not re-emit corChange when the already-selected tab is clicked', async () => {
      const onChange = vi.fn();
      const { root } = await render(
        <cor-tabs aria-label="Navigare" value="documente" onCorChange={onChange}></cor-tabs>,
      );
      await setTabs(root, sampleTabs);
      queryTabs(root)[1]!.click();
      await flush();
    ...
    it('does not emit corChange when a disabled tab is clicked', async () => {
      const onChange = vi.fn();
      const { root } = await render(<cor-tabs aria-label="Navigare" value="profil" onCorChange={onChange}></cor-tabs>);
      await setTabs(root, [
        { value: 'profil', label: 'Profil' },
    ...
    it('updates the selected tab when value prop changes externally', async () => {
      const { root } = await render(<cor-tabs aria-label="Navigare" value="profil"></cor-tabs>);
      await setTabs(root, sampleTabs);
  ...
  describe('keyboard contract (WAI-ARIA tabs)', () => {
    const pressKey = (host: HTMLElement, key: string, target?: HTMLElement) => {
      const focused = target ?? (host.querySelector('cor-tab[aria-selected="true"]') as HTMLElement | null);
    ...
    it('Home jumps selection to the first enabled tab', async () => {
      const { root } = await render(<cor-tabs aria-label="Navigare" value="setari"></cor-tabs>);
      await setTabs(root, sampleTabs);
  ...
  describe('roving tabindex', () => {
    ...
    it('disabled tabs always have tabindex -1', async () => {
      const { root } = await render(<cor-tabs aria-label="Navigare" value="a"></cor-tabs>);
      await setTabs(root, [
        { value: 'a', label: 'A' },
  ...
  describe('a11y wiring', () => {
    it('host carries role="tablist" and forwards aria-label', async () => {
      const { root } = await render(<cor-tabs aria-label="Cont"></cor-tabs>);
      expect(root?.getAttribute('role')).toBe('tablist');
      expect(root?.getAttribute('aria-label')).toBe('Cont');
    });

    it('each cor-tab carries role="tab" and aria-selected', async () => {
      const { root } = await render(<cor-tabs aria-label="Navigare" value="profil"></cor-tabs>);
      await setTabs(root, sampleTabs);
      const tabs = queryTabs(root);
    ...
    it('panels are wired with role="tabpanel" + aria-labelledby + hidden', async () => {
      const { root } = await render(
        <cor-tabs aria-label="Navigare" value="profil">
          <cor-tab value="profil" label="Profil"></cor-tab>
          <cor-tab value="documente" label="Documente"></cor-tab>
