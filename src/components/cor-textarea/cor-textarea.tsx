...
import type { TextareaChangeDetail, TextareaResize, TextareaSize, TextareaVariant } from './cor-textarea.types';
...
export class CorTextarea {
  ...
  @Event() corInput!: EventEmitter<TextareaChangeDetail>;

  /** Fires when the value is committed (typically on `blur`). `detail.value` is the committed value. */
  @Event() corChange!: EventEmitter<TextareaChangeDetail>;
  ...
  private readonly labelId = `cor-textarea-label-${this.instanceId}`;
  private readonly helperId = `cor-textarea-helper-${this.instanceId}`;
  ...
  validateVariant(next: TextareaVariant) {
    if (!TEXTAREA_VARIANTS.includes(next)) {
      console.warn(
        `[cor-textarea] variant="${String(next)}" is not supported. Supported: ${TEXTAREA_VARIANTS.join(
          ', ',
  ...
  validateSize(next: TextareaSize) {
    if (!TEXTAREA_SIZES.includes(next)) {
      console.warn(
        `[cor-textarea] size="${String(next)}" is not supported. Supported: ${TEXTAREA_SIZES.join(
          ', ',
        )}. Falling back to "md".`,
      );
      this.size = 'md';
  ...
  validateResize(next: TextareaResize) {
    if (!TEXTAREA_RESIZE.includes(next)) {
      console.warn(
        `[cor-textarea] resize="${String(next)}" is not supported. Supported: ${TEXTAREA_RESIZE.join(
          ', ',
