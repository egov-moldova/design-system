...
import { TOOLTIP_POSITIONS, TOOLTIP_SIZES, TOOLTIP_TRIGGERS, TOOLTIP_VARIANTS } from './cor-tooltip.types';
import type { TooltipPosition, TooltipSize, TooltipTrigger, TooltipVariant } from './cor-tooltip.types';
...
const docsSourceDefault = (args: TooltipArgs) => {
  const attrs = [
    ...
    .filter(Boolean)
    .join(' ');
  const open = attrs ? `<cor-tooltip ${attrs}>` : '<cor-tooltip>';
  return `${open}
  <button slot="trigger" type="button">${args.triggerLabel}</button>
  ${args.content}
</cor-tooltip>`;
...
export const FocusTrigger: Story = {
  ...
  parameters: {
    docs: {
      source: {
        code: `<cor-tooltip trigger="focus">
...
const renderAllSizes = () => /*html*/ `
  <div style="${gridStyle}">
    <div style="display: flex; flex-direction: column; align-items: center; gap: var(--spacing-12);">
      ...
      <cor-tooltip size="sm" trigger="manual" open>
    ...
    <div style="display: flex; flex-direction: column; align-items: center; gap: var(--spacing-12);">
      ...
      <cor-tooltip size="lg" trigger="manual" open>
        <button slot="trigger" type="button" style="${buttonStyle}">Detalii</button>
...
`;
const docsSourceAllSizes = /*html*/ `<cor-tooltip size="sm" trigger="manual" open>
  ...
  <div style="${positionGridStyle}">
    ${(['top', 'right', 'bottom', 'left'] as const)
      ...
      <div style="display: flex; flex-direction: column; align-items: center; gap: var(--spacing-12);">
        ...
        <cor-tooltip size="sm" position="${pos}" trigger="manual" open>
...
`;
const docsSourceAllPositions = /*html*/ `<cor-tooltip position="top" trigger="manual" open>
  <button slot="trigger" type="button">Trigger</button>
  Ajutor: poziție top.
</cor-tooltip>

<cor-tooltip position="right" trigger="manual" open>
  <button slot="trigger" type="button">Trigger</button>
  Ajutor: poziție right.
</cor-tooltip>

<cor-tooltip position="bottom" trigger="manual" open>
  <button slot="trigger" type="button">Trigger</button>
  Ajutor: poziție bottom.
</cor-tooltip>

<cor-tooltip position="left" trigger="manual" open>
  <button slot="trigger" type="button">Trigger</button>
  Ajutor: poziție left.
</cor-tooltip>`;
...
const renderAutoFlip = () => /*html*/ `
  <div style="display: flex; flex-direction: column; gap: var(--spacing-32); padding: var(--spacing-24);">
    ...
    <div style="display: flex; justify-content: center;">
      <cor-tooltip position="auto" trigger="manual" open>
...
`;
const docsSourceAutoFlip = /*html*/ `<cor-tooltip position="auto" trigger="manual" open>
...
const renderCoach = () => /*html*/ `
  <div style="${stageStyle}">
    <cor-tooltip size="lg" variant="coach" trigger="manual" open max-width="280">
...
`;
const docsSourceCoach = /*html*/ `<cor-tooltip size="lg" variant="coach" trigger="manual" open max-width="280">
...
const renderWithMaxWidth = () => /*html*/ `
  <div style="${stageStyle}">
    <cor-tooltip size="lg" trigger="manual" open max-width="320">
      <button slot="trigger" type="button" style="${buttonStyle}">Trigger</button>
      Tooltip cu o lățime maximă personalizată de 320 de pixeli. Folosit doar atunci când conținutul nu încape în 200px.
    </cor-tooltip>
  </div>
`;
const docsSourceWithMaxWidth = /*html*/ `<cor-tooltip size="lg" trigger="manual" open max-width="320">
  <button slot="trigger" type="button">Trigger</button>
...
`;
const docsSourceWithLongContent = /*html*/ `<cor-tooltip size="lg" trigger="manual" open>
  <button slot="trigger" type="button">Detalii suplimentare</button>
  Acesta este un tooltip cu mai multe linii de text care depășește lățimea unui singur rând.
</cor-tooltip>`;
...
const renderManual = () => /*html*/ `
  <div style="${stageStyle}">
    <cor-tooltip id="manual-tooltip" trigger="manual">
...
`;
const docsSourceManual = /*html*/ `<cor-tooltip trigger="manual" open>
...
`;
const docsSourceEdgeCases = /*html*/ `<cor-tooltip size="lg" trigger="manual" open>
