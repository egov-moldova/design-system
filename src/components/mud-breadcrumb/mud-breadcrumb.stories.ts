...
let storyInstance = 0;
const nextId = () => `mud-breadcrumb-story-${++storyInstance}`;
...
const docsSourceDefault = (args: BreadcrumbArgs) => `<mud-breadcrumb id="my-breadcrumb"
  max-visible="${args.maxVisible}"></mud-breadcrumb>
<script>
...
export const CoverageGuard: Story = {
  ...
  play: async () => {
    const ParentCtor = customElements.get('mud-breadcrumb') as unknown as
      | (new (registerHost: boolean) => unknown)
      | undefined;
    if (!ParentCtor) throw new Error('mud-breadcrumb constructor missing from registry');
    const parent = new ParentCtor(false);
    if (!parent) throw new Error('mud-breadcrumb did not construct');

    const ItemCtor = customElements.get('mud-breadcrumb-item') as unknown as
      | (new (registerHost: boolean) => unknown)
      | undefined;
    if (!ItemCtor) throw new Error('mud-breadcrumb-item constructor missing from registry');
    const item = new ItemCtor(false);
    if (!item) throw new Error('mud-breadcrumb-item did not construct');
  },
};
