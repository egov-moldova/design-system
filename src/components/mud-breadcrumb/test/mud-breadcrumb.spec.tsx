...
describe('mud-breadcrumb', () => {
  ...
  it('honors a custom aria-label', async () => {
    const { root } = await render(
      <mud-breadcrumb items={ROMANIAN_ITEMS} aria-label="Cale de navigare"></mud-breadcrumb>,
    );
  ...
  describe('overflow behavior', () => {
    ...
    it('collapses middle items into an overflow trigger when items > maxVisible', async () => {
      const { root } = await render(<mud-breadcrumb items={longItems} maxVisible={5}></mud-breadcrumb>);
  ...
  describe('events', () => {
    it('emits mudSelect when a non-active crumb is clicked', async () => {
      const onSelect = vi.fn();
      const { root, waitForChanges } = await render(
        <mud-breadcrumb items={ROMANIAN_ITEMS} onMudSelect={onSelect}></mud-breadcrumb>,
      );
    ...
    it('does NOT emit mudSelect when clicking the active crumb', async () => {
      const onSelect = vi.fn();
      const { root, waitForChanges } = await render(
        <mud-breadcrumb items={ROMANIAN_ITEMS} onMudSelect={onSelect}></mud-breadcrumb>,
      );
  ...
  describe('mobile responsive layout', () => {
    ...
    it('omits the mobile trail when responsive=false', async () => {
      const { root } = await render(<mud-breadcrumb items={ROMANIAN_ITEMS} responsive={false}></mud-breadcrumb>);
    ...
    it('uses default slot when items prop is omitted', async () => {
      const { root } = await render(
        <mud-breadcrumb>
          ...
          <mud-breadcrumb-item active>Profil</mud-breadcrumb-item>
        </mud-breadcrumb>,
      );
  ...
  describe('long-label tooltip (Figma "Best Practices")', () => {
    it('wraps labels > 30 characters in <mud-tooltip slot="trigger">', async () => {
      ...
      const { root } = await render(<mud-breadcrumb items={items}></mud-breadcrumb>);
      const tooltip = root?.shadowRoot?.querySelector<HTMLElement & { content?: string }>('mud-tooltip');
      expect(tooltip).toBeTruthy();
      // The trigger slot wraps the visible (truncated) crumb body
      expect(tooltip?.querySelector('[slot="trigger"]')).toBeTruthy();
      // The full label is available via the mud-tooltip's default slot content
    ...
    it('does NOT wrap labels at or below the 30-char threshold', async () => {
      ...
      const { root } = await render(<mud-breadcrumb items={items}></mud-breadcrumb>);
      expect(root?.shadowRoot?.querySelector('mud-tooltip')).toBeFalsy();
    });
  });
  ...
  describe('separator', () => {
    it('renders text-mode separators when a non-empty `separator` prop is set', async () => {
      ...
      const { root } = await render(<mud-breadcrumb items={items} separator="/"></mud-breadcrumb>);
    ...
    it('clones slotted `slot="separator"` content via the cached template', async () => {
      const { root } = await render(
        <mud-breadcrumb items={ROMANIAN_ITEMS}>
  ...
  describe('overflow menu keyboard', () => {
    ...
    it('ArrowDown sets aria-activedescendant on the trigger', async () => {
      ...
      await waitForChanges();
      expect(trigger?.getAttribute('aria-activedescendant')).toBe('mud-bc-overflow-0');
    ...
    it('End jumps to the last menu item', async () => {
      ...
      await waitForChanges();
      expect(trigger?.getAttribute('aria-activedescendant')).toBe('mud-bc-overflow-4');
    });
  ...
  describe('aria-label render-loop safety', () => {
    ...
    it('falls back to the `label` prop when aria-label is not provided', async () => {
      const { root } = await render(<mud-breadcrumb items={ROMANIAN_ITEMS} label="Calea"></mud-breadcrumb>);
  ...
  describe('edge cases', () => {
    it('coerces maxVisible to 2 when set to NaN', async () => {
      const { root, waitForChanges } = await render(
        <mud-breadcrumb items={ROMANIAN_ITEMS} maxVisible={5}></mud-breadcrumb>,
      );
    ...
    it('selects from the overflow menu and emits with fromOverflow=true', async () => {
      ...
      const onSelect = vi.fn();
      const { root, waitForChanges } = await render(
        <mud-breadcrumb items={longItems} maxVisible={5} onMudSelect={onSelect}></mud-breadcrumb>,
      );
      const trigger = root?.shadowRoot?.querySelector<HTMLButtonElement>('.overflow-trigger');
      trigger?.click();
      await waitForChanges();
    ...
    it('emits with fromOverflow=true via a non-link overflow item (button)', async () => {
      ...
      const onSelect = vi.fn();
      const { root, waitForChanges } = await render(
        <mud-breadcrumb items={items} maxVisible={3} onMudSelect={onSelect}></mud-breadcrumb>,
      );
      const trigger = root?.shadowRoot?.querySelector<HTMLButtonElement>('.overflow-trigger');
      trigger?.click();
      await waitForChanges();
    ...
    it('renders the default slot when items is undefined or empty', async () => {
      const { root } = await render(<mud-breadcrumb></mud-breadcrumb>);
    ...
    it('omits the mobile back link when there are no items to fall back to', async () => {
      ...
      const { root } = await render(<mud-breadcrumb items={single}></mud-breadcrumb>);
      const back = root?.shadowRoot?.querySelector('.back-link');
    ...
    it('honours preventDefault by the consumer on mudSelect (does not navigate)', async () => {
...
describe('mud-breadcrumb-item', () => {
  ...
  it('renders as a span when active is true (no link)', async () => {
    const { root } = await render(
      <mud-breadcrumb-item href="/x" active>
        Current
      </mud-breadcrumb-item>,
    );
  ...
  it('reflects visited prop', async () => {
    const { root } = await render(
      <mud-breadcrumb-item href="/x" visited>
        Visited
      </mud-breadcrumb-item>,
    );
  ...
  it('reflects disabled and renders as span with aria-disabled', async () => {
    const { root } = await render(
      <mud-breadcrumb-item href="/x" disabled>
        Off
      </mud-breadcrumb-item>,
    );
  ...
  it('renders a spinner instead of the label when loading', async () => {
    const { root } = await render(
      <mud-breadcrumb-item loading label="Loading">
        Loading
      </mud-breadcrumb-item>,
    );
    expect(root?.shadowRoot?.querySelector('mud-spinner')).toBeTruthy();
    expect(root?.getAttribute('aria-busy')).toBe('true');
  });

  it('fires mudSelect on click of an interactive crumb', async () => {
    const onSelect = vi.fn();
    const { root, waitForChanges } = await render(
      <mud-breadcrumb-item href="/x" label="Test" onMudSelect={onSelect}>
        Test
      </mud-breadcrumb-item>,
    );
    const link = root?.shadowRoot?.querySelector<HTMLAnchorElement>('a.crumb');
  ...
  it('does not fire mudSelect when disabled is true', async () => {
    const onSelect = vi.fn();
    const { root, waitForChanges } = await render(
      <mud-breadcrumb-item href="/x" disabled onMudSelect={onSelect}>
        Off
      </mud-breadcrumb-item>,
    );
  ...
  it('fires mudSelect on click when the crumb is a non-link span', async () => {
    const onSelect = vi.fn();
    const { root, waitForChanges } = await render(
      <mud-breadcrumb-item label="Plain" onMudSelect={onSelect}>
        Plain
      </mud-breadcrumb-item>,
    );
    const span = root?.shadowRoot?.querySelector<HTMLSpanElement>('span.crumb');
  ...
  it('wraps long labels in a <mud-tooltip slot="trigger">', async () => {
    const longLabel = 'This breadcrumb label is comfortably longer than the 30-char limit';
    const { root } = await render(
      <mud-breadcrumb-item href="/long" label={longLabel}>
        {longLabel}
      </mud-breadcrumb-item>,
    );
    const tooltip = root?.shadowRoot?.querySelector('mud-tooltip');
    expect(tooltip).toBeTruthy();
    expect(tooltip?.querySelector('[slot="trigger"]')).toBeTruthy();
    expect(tooltip?.textContent ?? '').toContain(longLabel);
  });
  ...
  it('ignores unrelated keys on the crumb', async () => {
    const onSelect = vi.fn();
    const { root, waitForChanges } = await render(
      <mud-breadcrumb-item href="/x" onMudSelect={onSelect}>
        X
      </mud-breadcrumb-item>,
    );
    const link = root?.shadowRoot?.querySelector<HTMLAnchorElement>('a.crumb');
  ...
  it('keydown returns early when disabled', async () => {
    const onSelect = vi.fn();
    const { root, waitForChanges } = await render(
      <mud-breadcrumb-item disabled onMudSelect={onSelect}>
        Off
      </mud-breadcrumb-item>,
    );
    const span = root?.shadowRoot?.querySelector<HTMLSpanElement>('span.crumb');
  ...
  it('honours consumer preventDefault on mudSelect', async () => {
    const onSelect = vi.fn((ev: CustomEvent) => ev.preventDefault());
    const { root, waitForChanges } = await render(
      <mud-breadcrumb-item href="/x" label="X" onMudSelect={onSelect}>
        X
      </mud-breadcrumb-item>,
    );
    const link = root?.shadowRoot?.querySelector<HTMLAnchorElement>('a.crumb');
