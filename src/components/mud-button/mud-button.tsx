...
export class MudButton {
   ...
   * stretches to 100% width — designed for use inside `mud-button-group`
  ...
  componentDidLoad() {
    if (this.iconOnly) {
      ...
      if (!this.hasIcon) {
        console.warn(
          '[mud-button] icon-only is set but no `<mud-icon slot="icon">` was provided. The button will render an empty square.',
        );
      }
    }
    if (
      ...
      console.warn(
        `[mud-button] appearance="${this.appearance}" does not support variant="${this.variant}". ` +
