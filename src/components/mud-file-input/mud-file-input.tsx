...
@Component({
  tag: 'mud-file-input',
  styleUrl: 'mud-file-input.css',
  shadow: { delegatesFocus: true },
  formAssociated: true,
})
export class MudFileInput {
  /**
  ...
  @Event() mudChange!: EventEmitter<FileInputChangeDetail>;

  /** Fires when a drag enters the drop zone. */
  @Event() mudDragEnter!: EventEmitter<DragEvent>;

  /** Fires when the drag leaves the drop zone. */
  @Event() mudDragLeave!: EventEmitter<DragEvent>;

  /** Fires after a drop, with the accepted / rejected split + the first rejection reason. */
  @Event() mudDrop!: EventEmitter<FileInputDropDetail>;

  /** Fires when a file is removed from the inline list. */
  @Event() mudRemove!: EventEmitter<FileInputRemoveDetail>;

  /** Fires for every rejected file (size / type / count). One event per file. */
  @Event() mudError!: EventEmitter<FileInputErrorDetail>;

  private readonly instanceId = ++fileInputInstanceCounter;
  private readonly labelId = `mud-file-input-label-${this.instanceId}`;
  private readonly helperId = `mud-file-input-helper-${this.instanceId}`;
  private readonly errorId = `mud-file-input-error-${this.instanceId}`;
  private readonly dropzoneId = `mud-file-input-dropzone-${this.instanceId}`;
  private readonly liveId = `mud-file-input-live-${this.instanceId}`;
  ...
  validateSize(next: FileInputSize) {
    if (!FILE_INPUT_SIZES.includes(next)) {
      console.warn(
        `[mud-file-input] size="${String(next)}" is not supported. Supported: ${FILE_INPUT_SIZES.join(
          ', ',
        )}. Falling back to "md".`,
      );
      this.size = 'md';
  ...
  private intakeFiles(
    ...
    for (const file of incoming) {
      ...
      if (validity !== null) {
        ...
        this.mudError.emit({ code: validity, message: this.reasonMessage(validity, file), file });
        continue;
      }
      if (this.maxFiles !== undefined && remaining <= 0) {
        rejected.push(file);
        firstReason ??= 'count';
        this.mudError.emit({ code: 'count', message: this.reasonMessage('count', file), file });
        continue;
      }
      accepted.push(file);
      remaining -= 1;
    ...
    if (source === 'drop') {
      this.mudDrop.emit({ accepted, rejected, reason: firstReason });
    }
