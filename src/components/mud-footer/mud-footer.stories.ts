...
import { FOOTER_LOCALES, FOOTER_VARIANTS } from './mud-footer.types';
import type { FooterLocale, FooterVariant } from './mud-footer.types';
...
Composes \`mud-link\`, \`mud-logo\` and \`mud-icon\` for atomic pieces. Two
...
The \`mudLocaleChange\` event fires with \`{ locale }\` when the user selects
...
export const WithCustomSections: Story = {
  ...
  render: (args: FooterArgs) => {
    const id = `mud-footer-custom-${Math.random().toString(36).slice(2, 8)}`;
    requestAnimationFrame(() => {
      const el = document.getElementById(id) as HTMLMudFooterElement | null;
      if (!el) return;
      el.sections = [
        {
          title: 'MPay',
...
export const EdgeCases: Story = {
  ...
  render: (args: FooterArgs) => {
    const id = `mud-footer-edge-${Math.random().toString(36).slice(2, 8)}`;
    requestAnimationFrame(() => {
      const el = document.getElementById(id) as HTMLMudFooterElement | null;
      if (!el) return;
      el.sections = [
