...
| `maxChips`        | `max-chips`        | Maximum number of chips accepted. Further additions emit `mudError` with `code: 'max'`.                                                 | `number \| undefined`        | `undefined` |
...
| --------------- | ---------------------------------------------------------- | ------------------------------------ |
| `mudBlur`       | Fires when the inline input loses focus.                   | `CustomEvent<FocusEvent>`            |
| `mudChange`     | Fires whenever the chip array changes (add or remove).     | `CustomEvent<InputChipChangeDetail>` |
| `mudChipAdd`    | Fires when a chip is successfully added.                   | `CustomEvent<InputChipAddDetail>`    |
| `mudChipRemove` | Fires when a chip is removed from the list.                | `CustomEvent<InputChipRemoveDetail>` |
| `mudError`      | Fires for every rejected chip (pattern / duplicate / max). | `CustomEvent<InputChipErrorDetail>`  |
| `mudFocus`      | Fires when the inline input gains focus.                   | `CustomEvent<FocusEvent>`            |
...
graph TD;
  ...
  style mud-input-chip fill:#f9f,stroke:#333,stroke-width:4px
```
