...
const docsSourceDefault = (args: LogoArgs) => {
  const attrs = [`name="${args.name}"`, args.ariaLabel ? `aria-label="${args.ariaLabel}"` : '']
    .filter(Boolean)
    .join(' ');
  return `<mud-logo ${attrs}></mud-logo>`;
};

const docsSourceAllLogos = SERVICES.flatMap(s =>
  LAYOUTS.map(l => /*html*/ `<mud-logo name="${s}-logo-${l}"></mud-logo>`),
).join('\n');

const docsSourceLogomarks = SERVICES.map(s => /*html*/ `<mud-logo name="${s}-logo-logomark-only"></mud-logo>`).join(
  '\n',
);
...
export const Default: Story = {
  ...
  parameters: {
    docs: {
      source: {
        type: 'dynamic',
        // Returns the minimal `<mud-logo></mud-logo>` markup a consumer would
        ...
        LAYOUTS.map(
          l => /*html*/ `
              <div style="${cellStyle}">
                ...
                <mud-logo name="${s}-logo-${l}"></mud-logo>
              </div>
            `,
        ),
      ).join('')}
      ...
      ${SERVICES.map(
        s => /*html*/ `
          <div style="${cellStyle} align-items: center;">
            <mud-logo name="${s}-logo-logomark-only"></mud-logo>
