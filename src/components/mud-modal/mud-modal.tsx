 ...
 * @slot actions - Footer action group. Typically `mud-button` instances.
 */
@Component({
  tag: 'mud-modal',
  styleUrl: 'mud-modal.css',
  shadow: true,
})
export class MudModal {
  /**
   * Whether the modal is currently shown. Reflected so consumers can target
   * `mud-modal[open]` in selectors. Mutable so the component can flip it back
   ...
   * it emits `mudClose` with `reason: 'close-button'`. Hide it for required
   ...
   * with a destructive primary `mud-button` in the actions slot.
   * @default false
   */
  ...
  @Element() host!: HTMLMudModalElement;
   ...
   */
  @Event() mudClose!: EventEmitter<ModalCloseEvent>;
  ...
  private handleDialogClose = () => {
    ...
    if (this.open) {
      ...
      this.open = false;
      this.mudClose.emit({ reason: 'close-button' });
    }
  };
