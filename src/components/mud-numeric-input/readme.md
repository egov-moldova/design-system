...
| `ariaValuetext`  | `aria-valuetext`  | Human-readable value announcement for screen readers (e.g. `"5 lei"`). Maps to the native `aria-valuetext` on the spinbutton. An `aria-valuetext` attribute on the host is read once on load and stripped; later updates go through this prop, and setting it empty does not clear the value.               | `string \| undefined`                     | `undefined` |
