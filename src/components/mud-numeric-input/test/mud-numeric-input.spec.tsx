...
describe('mud-numeric-input', () => {
  describe('defaults + prop reflection', () => {
    it('renders with default props reflected on host', async () => {
      const { root } = await render(<mud-numeric-input label="Quantity"></mud-numeric-input>);
    ...
    it.each(NUMERIC_INPUT_VARIANTS)('reflects variant="%s" to host', async variant => {
      const { root } = await render(<mud-numeric-input variant={variant} label="x"></mud-numeric-input>);
      expect(root?.getAttribute('variant')).toBe(variant);
    });

    it.each(NUMERIC_INPUT_SIZES)('reflects size="%s" to host', async size => {
      const { root } = await render(<mud-numeric-input size={size} label="x"></mud-numeric-input>);
      expect(root?.getAttribute('size')).toBe(size);
    });

    it('warns and falls back when variant is invalid', async () => {
      const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
      const { root } = await render(<mud-numeric-input label="x"></mud-numeric-input>);
  ...
  describe('shadow structure', () => {
    ...
    it('renders a helper assistive row when `helper-text` is set', async () => {
      const { root } = await render(<mud-numeric-input label="x" helper-text="0–10"></mud-numeric-input>);
    ...
    it('renders an error assistive row when invalid + error-text', async () => {
      const { root } = await render(
        <mud-numeric-input label="x" invalid error-text="Out of range"></mud-numeric-input>,
      );
      const assistive = queryAssistive(root);
    ...
    it('renders both stepper buttons when show-steppers is opted in', async () => {
      const { root } = await render(<mud-numeric-input label="x" show-steppers></mud-numeric-input>);
  ...
  describe('value + form association', () => {
    it('renders the value via the display field', async () => {
      const { root } = await render(<mud-numeric-input label="x" value={42}></mud-numeric-input>);
      expect(queryNative(root)?.value).toBe('42');
    });

    it('reflects value to the host attribute when set', async () => {
      const { root } = await render(<mud-numeric-input label="x" value={7}></mud-numeric-input>);
      expect(root?.getAttribute('value')).toBe('7');
    });

    it('emits mudInput on each keystroke with parsed value', async () => {
      const onInput = vi.fn();
      const { root } = await render(<mud-numeric-input label="x" onMudInput={onInput}></mud-numeric-input>);
      const native = queryNative(root)!;
      native.value = '12';
    ...
    it('emits mudInput with `null` when the user types a non-numeric residue', async () => {
      const onInput = vi.fn();
      const { root } = await render(<mud-numeric-input label="x" onMudInput={onInput}></mud-numeric-input>);
      const native = queryNative(root)!;
      native.value = '-';
    ...
    it('emits mudChange on blur with the committed value', async () => {
    ...
    it('emits mudFocus and mudBlur and toggles the is-focused class', async () => {
      ...
      const onBlur = vi.fn();
      const { root } = await render(
        <mud-numeric-input label="x" onMudFocus={onFocus} onMudBlur={onBlur}></mud-numeric-input>,
      );
      const native = queryNative(root)!;
  ...
  describe('clamping + precision on commit', () => {
    it('clamps below min on blur', async () => {
      const onChange = vi.fn();
      const { root } = await render(
        <mud-numeric-input label="x" min={0} max={10} onMudChange={onChange}></mud-numeric-input>,
      );
      const native = queryNative(root)!;
      native.value = '-5';
      native.dispatchEvent(new Event('input', { bubbles: true }));
    ...
    it('rounds to precision on blur', async () => {
      const onChange = vi.fn();
      const { root } = await render(
        <mud-numeric-input label="x" precision={2} onMudChange={onChange}></mud-numeric-input>,
      );
      const native = queryNative(root)!;
    ...
    it('emits mudError when typed value is out of range', async () => {
      const onError = vi.fn();
      const { root } = await render(
        <mud-numeric-input label="x" min={0} max={10} onMudError={onError}></mud-numeric-input>,
      );
      const native = queryNative(root)!;
      native.value = '20';
      native.dispatchEvent(new Event('input', { bubbles: true }));
  ...
  describe('stepper buttons', () => {
    ...
    it('disables stepper-up at max', async () => {
      const { root } = await render(
        <mud-numeric-input label="x" show-steppers value={10} min={0} max={10}></mud-numeric-input>,
      );
    ...
    it('disables stepper-down at min', async () => {
      const { root } = await render(
        <mud-numeric-input label="x" show-steppers value={0} min={0} max={10}></mud-numeric-input>,
      );
    ...
    it('seeds the value from `min` when the field is empty and stepper-up is pressed', async () => {
      const onChange = vi.fn();
      const { root } = await render(
        <mud-numeric-input label="x" show-steppers min={5} max={20} onMudChange={onChange}></mud-numeric-input>,
      );
      const up = queryStepperUp(root)!;
      up.click();
      await flush();
  ...
  describe('keyboard contract', () => {
    ...
    it('increments on ArrowUp', async () => {
      const onChange = vi.fn();
      const { root } = await render(<mud-numeric-input label="x" value={5} onMudChange={onChange}></mud-numeric-input>);
      press(root, 'ArrowUp');
      await flush();
      expect(onChange.mock.calls.at(-1)?.[0].detail).toEqual({ value: 6 });
    });
    ...
    it('ignores ArrowUp/Down when disabled', async () => {
      const onChange = vi.fn();
      const { root } = await render(
        <mud-numeric-input label="x" value={5} disabled onMudChange={onChange}></mud-numeric-input>,
      );
      press(root, 'ArrowUp');
      await flush();
    ...
    it('ignores ArrowUp/Down when readonly', async () => {
      const onChange = vi.fn();
      const { root } = await render(
        <mud-numeric-input label="x" value={5} readonly onMudChange={onChange}></mud-numeric-input>,
      );
      press(root, 'ArrowUp');
      await flush();
      expect(onChange).not.toHaveBeenCalled();
  ...
  describe('disabled + readonly behavior', () => {
    it('passes disabled through to the native input (native attr only — no redundant aria-disabled)', async () => {
      const { root } = await render(<mud-numeric-input label="x" disabled></mud-numeric-input>);
      const native = queryNative(root);
    ...
    it('hides the stepper stack when readonly', async () => {
      const { root } = await render(<mud-numeric-input label="x" readonly value={1}></mud-numeric-input>);
      expect(queryStepperUp(root)).toBeNull();
      expect(queryStepperDown(root)).toBeNull();
    });

    it('passes readonly through to the native input', async () => {
      const { root } = await render(<mud-numeric-input label="x" readonly value={5}></mud-numeric-input>);
  ...
  describe('ARIA contract', () => {
    ...
    it('exposes aria-valuemin / aria-valuemax', async () => {
      const { root } = await render(<mud-numeric-input label="x" min={0} max={100}></mud-numeric-input>);
      const native = queryNative(root);
    ...
    it('forwards aria-valuetext to the native input', async () => {
      const { root } = await render(<mud-numeric-input label="x" value={5} ariaValuetext="5 lei"></mud-numeric-input>);
    ...
    it('exposes aria-invalid when invalid', async () => {
      const { root } = await render(<mud-numeric-input label="x" invalid></mud-numeric-input>);
    ...
    it('honors custom increment / decrement labels', async () => {
      const { root } = await render(
        <mud-numeric-input label="x" show-steppers increment-label="Plus" decrement-label="Minus"></mud-numeric-input>,
      );
    ...
    it('forwards content into the icon-start slot', async () => {
      const { root } = await render(
        <mud-numeric-input label="Sum">
          <mud-icon slot="icon-start" name="wallet" size={20}></mud-icon>
        </mud-numeric-input>,
      );
  ...
  describe('form lifecycle', () => {
    it('restores the initial value on formResetCallback', async () => {
      const { root } = await render(<mud-numeric-input label="x" value={3}></mud-numeric-input>);
  ...
  describe('loading state', () => {
    it('reflects loading to host and sets aria-busy', async () => {
      const { root } = await render(<mud-numeric-input label="x" loading></mud-numeric-input>);
    ...
    it('scales the spinner from xs (md) to sm (lg)', async () => {
      const { root: rootMd } = await render(<mud-numeric-input label="x" loading size="md"></mud-numeric-input>);
      const { root: rootLg } = await render(<mud-numeric-input label="x" loading size="lg"></mud-numeric-input>);
    ...
    it('ignores ArrowUp/Down when loading', async () => {
      ...
      const onChange = vi.fn();
      const { root } = await render(
        <mud-numeric-input label="x" value={5} loading onMudChange={onChange}></mud-numeric-input>,
      );
      const instance = root as unknown as KeyHandlerInstance;
  ...
  describe('variant matrix', () => {
    it.each(NUMERIC_INPUT_VARIANTS)('reflects variant="%s" to host without warning', async variant => {
      const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
      const { root } = await render(<mud-numeric-input label="x" variant={variant}></mud-numeric-input>);
      expect(root?.getAttribute('variant')).toBe(variant);
  ...
  describe('form validity', () => {
    ...
    it('sets valueMissing on transition from filled to empty when required', async () => {
      const { root } = await render(<mud-numeric-input label="x" required value={5}></mud-numeric-input>);
    ...
    it('reports rangeUnderflow when value drops below min', async () => {
      const { root } = await render(<mud-numeric-input label="x" min={0} value={5}></mud-numeric-input>);
      const internals = (root as unknown as InstanceWithInternals).internals;
      const spy = vi.spyOn(internals, 'setValidity');
    ...
    it('reports rangeOverflow when value exceeds max', async () => {
      const { root } = await render(<mud-numeric-input label="x" max={10} value={5}></mud-numeric-input>);
      const internals = (root as unknown as InstanceWithInternals).internals;
      const spy = vi.spyOn(internals, 'setValidity');
    ...
    it('uses error-text as the validation message when set', async () => {
      const { root } = await render(
        <mud-numeric-input label="x" required value={5} error-text="Câmp obligatoriu"></mud-numeric-input>,
      );
      const internals = (root as unknown as InstanceWithInternals).internals;
      const spy = vi.spyOn(internals, 'setValidity');
      (root as unknown as { value: number | undefined }).value = undefined;
