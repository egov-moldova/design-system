...
import '../mud-search-input-circular';
...
describe('mud-search-input-circular', () => {
  describe('defaults + prop reflection', () => {
    it('renders with default props reflected on host', async () => {
      const { root } = await render(<mud-search-input-circular></mud-search-input-circular>);
    ...
    it.each(SEARCH_INPUT_CIRCULAR_VARIANTS)('reflects variant="%s" to host', async variant => {
      const { root } = await render(<mud-search-input-circular variant={variant}></mud-search-input-circular>);
      expect(root?.getAttribute('variant')).toBe(variant);
    });

    it.each(SEARCH_INPUT_CIRCULAR_SIZES)('reflects size="%s" to host', async size => {
      const { root } = await render(<mud-search-input-circular size={size}></mud-search-input-circular>);
  ...
  describe('shadow structure', () => {
    it('renders the label via the `label` prop', async () => {
      const { root } = await render(<mud-search-input-circular label="Căutare"></mud-search-input-circular>);
      const label = queryLabel(root);
      expect(label?.textContent).toContain('Căutare');
    });

    it('adds a required mark when `required` is set', async () => {
      const { root } = await render(<mud-search-input-circular label="x" required></mud-search-input-circular>);
    ...
    it('respects the iconName prop on the default leading icon', async () => {
      const { root } = await render(<mud-search-input-circular icon-name="filter"></mud-search-input-circular>);
      const icon = root?.shadowRoot?.querySelector('.control-icon-start mud-icon');
    ...
    it('renders a helper assistive row when `helper-text` is set', async () => {
      const { root } = await render(
        <mud-search-input-circular label="x" helper-text="hint"></mud-search-input-circular>,
      );
    ...
    it('renders an error assistive row with the error icon when invalid + error-text', async () => {
      const { root } = await render(
        <mud-search-input-circular label="x" invalid error-text="Required"></mud-search-input-circular>,
      );
      const assistive = queryAssistive(root);
  ...
  describe('clear button visibility', () => {
    ...
    it('shows the clear button when value is non-empty', async () => {
      const { root } = await render(<mud-search-input-circular value="hello"></mud-search-input-circular>);
    ...
    it('uses the custom clearLabel for aria-label', async () => {
      const { root } = await render(
        <mud-search-input-circular value="hello" clear-label="Clear"></mud-search-input-circular>,
      );
    ...
    it('hides the clear button when clearable=false', async () => {
      const { root } = await render(
        <mud-search-input-circular value="hello" clearable={false}></mud-search-input-circular>,
      );
      expect(queryClearButton(root)).toBeNull();
    });

    it('hides the clear button when disabled', async () => {
      const { root } = await render(<mud-search-input-circular value="hello" disabled></mud-search-input-circular>);
      expect(queryClearButton(root)).toBeNull();
    });

    it('hides the clear button when readonly', async () => {
      const { root } = await render(<mud-search-input-circular value="hello" readonly></mud-search-input-circular>);
      expect(queryClearButton(root)).toBeNull();
    });
  });
  ...
  describe('value + form association', () => {
    ...
    it('emits mudInput on each keystroke', async () => {
      const onInput = vi.fn();
      const { root } = await render(<mud-search-input-circular onMudInput={onInput}></mud-search-input-circular>);
      const native = queryNative(root)!;
      native.value = 'a';
    ...
    it('emits mudChange on change (blur commit)', async () => {
      ...
      const { root } = await render(<mud-search-input-circular onMudChange={onChange}></mud-search-input-circular>);
      const native = queryNative(root)!;
      native.value = 'done';
    ...
    it('emits mudFocus and mudBlur and toggles the is-focused class', async () => {
      ...
      const onBlur = vi.fn();
      const { root } = await render(
        <mud-search-input-circular onMudFocus={onFocus} onMudBlur={onBlur}></mud-search-input-circular>,
      );
      const native = queryNative(root)!;
  ...
  describe('keyboard contract', () => {
    ...
    it('emits mudSearch on Enter with the current value', async () => {
      ...
      const { root } = await render(
        <mud-search-input-circular value="hello" onMudSearch={onSearch}></mud-search-input-circular>,
      );
      press(root, 'Enter');
      await flush();
    ...
    it('Escape with empty value does not emit mudClear', async () => {
      const onClear = vi.fn();
      const { root } = await render(<mud-search-input-circular onMudClear={onClear}></mud-search-input-circular>);
      press(root, 'Escape');
      await flush();
    ...
    it('Escape on disabled input does not clear', async () => {
      const onClear = vi.fn();
      const { root } = await render(
        <mud-search-input-circular value="hello" disabled onMudClear={onClear}></mud-search-input-circular>,
      );
      press(root, 'Escape');
      await flush();
      expect(onClear).not.toHaveBeenCalled();
  ...
  describe('disabled + readonly behavior', () => {
    it('passes disabled through to the native input', async () => {
      const { root } = await render(<mud-search-input-circular disabled></mud-search-input-circular>);
      const native = queryNative(root);
    ...
    it('passes readonly through to the native input', async () => {
      const { root } = await render(<mud-search-input-circular readonly value="x"></mud-search-input-circular>);
      const native = queryNative(root);
  ...
  describe('ARIA contract', () => {
    ...
    it('uses aria-label as the accessible name when no visible label is present', async () => {
      const { root } = await render(<mud-search-input-circular aria-label="Caută"></mud-search-input-circular>);
      const native = queryNative(root);
    ...
    it('exposes aria-invalid when invalid', async () => {
      const { root } = await render(<mud-search-input-circular invalid></mud-search-input-circular>);
  ...
  describe('slots', () => {
    it('forwards content into the icon-end slot', async () => {
      const { root } = await render(
        <mud-search-input-circular value="hello">
  ...
  describe('loading state', () => {
    it('reflects loading to the host attribute', async () => {
      const { root } = await render(<mud-search-input-circular loading></mud-search-input-circular>);
    ...
    it('uses spinner size "md" on size="lg" and "sm" on size="md"', async () => {
      const { root: lg } = await render(<mud-search-input-circular size="lg" loading></mud-search-input-circular>);
      expect(querySpinner(lg)?.getAttribute('size')).toBe('md');
      const { root: md } = await render(<mud-search-input-circular size="md" loading></mud-search-input-circular>);
    ...
    it('suppresses the clear button while loading (Figma master 933:29721)', async () => {
      const { root } = await render(<mud-search-input-circular value="hello" loading></mud-search-input-circular>);
      expect(queryClearButton(root)).toBeNull();
    });
  });
  ...
  describe('with-button (submit affordance)', () => {
    it('reflects withButton via the `with-button` attribute', async () => {
      const { root } = await render(<mud-search-input-circular with-button></mud-search-input-circular>);
    ...
    it('uses the custom submitLabel for aria-label', async () => {
      const { root } = await render(
        <mud-search-input-circular with-button submit-label="Search"></mud-search-input-circular>,
      );
    ...
    it('the submit button is enabled when the value is non-empty', async () => {
      const { root } = await render(<mud-search-input-circular with-button value="hello"></mud-search-input-circular>);
      const button = querySubmitButton(root)!;
    ...
    it('the submit button is disabled when the host is disabled', async () => {
      const { root } = await render(
        <mud-search-input-circular with-button value="hello" disabled></mud-search-input-circular>,
      );
    ...
    it('the submit button is disabled when the host is readonly', async () => {
      const { root } = await render(
        <mud-search-input-circular with-button value="hello" readonly></mud-search-input-circular>,
      );
      expect(querySubmitButton(root)?.hasAttribute('disabled')).toBe(true);
    });
    ...
    it('emits mudSearch on submit-button click with the current value', async () => {
      const onSearch = vi.fn();
      const { root } = await render(
        <mud-search-input-circular with-button value="hello" onMudSearch={onSearch}></mud-search-input-circular>,
      );
    ...
    it('coexists with the loading spinner when both are set', async () => {
      const { root } = await render(
        <mud-search-input-circular with-button value="hello" loading></mud-search-input-circular>,
      );
      expect(querySpinner(root)).toBeTruthy();
      expect(querySubmitButton(root)).toBeTruthy();
    });
  ...
  describe('form validity', () => {
    ...
    it('sets valueMissing on transition from filled to empty when required', async () => {
      const { root } = await render(
        <mud-search-input-circular label="x" required value="query"></mud-search-input-circular>,
      );
    ...
    it('re-syncs validity when required toggles', async () => {
      const { root } = await render(<mud-search-input-circular label="x"></mud-search-input-circular>);
      const internals = (root as unknown as InstanceWithInternals).internals;
      const spy = vi.spyOn(internals, 'setValidity');
