...
| `emptyLabel`      | `empty-label`      | Shown in place of the list when nothing matches the query.                                                                                                                                                                                                                 | `string`                     | `'Nicio opțiune'`        |
...
| `listboxLabel`    | `listbox-label`    | Names the listbox for assistive technology when the field has no visible label and no `aria-label` to borrow.                                                                                                                                                              | `string`                     | `'Opțiuni'`              |
...
| `requiredMessage` | `required-message` | Validation message reported when the field is `required` and nothing is selected.                                                                                                                                                                                          | `string`                     | `'Selectați o opțiune.'` |
| `searchable`      | `searchable`       | Lets the user narrow the list by typing into the control.  Off by default: turning it on makes the control a text field, which changes how every existing select behaves — including raising an on-screen keyboard on touch — so it is the consumer's call, not a default. | `boolean`                    | `false`                  |
...
| Slot           | Description                                                                                                                                                                                                                                                  |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
|                | (default) The option list, written as the markup a native `<select>` takes: `<option>`, `<optgroup label="…">` and `<hr>`. Not rendered directly — each option's `value`, text, `disabled` and `selected` are read, and re-read whenever the markup changes. |
