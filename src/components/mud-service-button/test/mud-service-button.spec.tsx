...
describe('mud-service-button', () => {
  ...
  it('reflects appearance, disabled, loading, full-width to host attributes', async () => {
    const { root } = await render(
      <mud-service-button appearance="neutral" disabled loading full-width>
        Test
      </mud-service-button>,
    );
  ...
  it('renders <a> instead of <button> when href is set', async () => {
    const { root, waitForChanges } = await render(
      <mud-service-button href="https://mpay.gov.md" target="_blank" rel="noopener">
        Pay
      </mud-service-button>,
    );
    await waitForChanges();
  ...
  it('strips href when inert (disabled) to prevent navigation', async () => {
    const { root, waitForChanges } = await render(
      <mud-service-button href="https://mpay.gov.md" disabled>
        Pay
      </mud-service-button>,
    );
    await waitForChanges();
  ...
  it('disabled state: native disabled attribute + aria-disabled=true', async () => {
    const { root, waitForChanges } = await render(<mud-service-button disabled>Pay</mud-service-button>);
    await waitForChanges();
  ...
  it('loading state: aria-busy=true, spinner shown, badge+label remain in DOM (CSS hides visually to preserve width)', async () => {
    const { root, waitForChanges } = await render(<mud-service-button loading>Pay</mud-service-button>);
    await waitForChanges();
  ...
  it('loading: spinner variant is light-on-color for primary, dark for neutral', async () => {
    const { root: rootPrimary, waitForChanges: waitPrimary } = await render(
      <mud-service-button loading appearance="primary">
        Pay
      </mud-service-button>,
    );
    await waitPrimary();
    expect(rootPrimary?.shadowRoot?.querySelector('mud-spinner')?.getAttribute('variant')).toBe('light-on-color');

    const { root: rootNeutral, waitForChanges: waitNeutral } = await render(
      <mud-service-button loading appearance="neutral">
        Pay
      </mud-service-button>,
    );
    await waitNeutral();
    expect(rootNeutral?.shadowRoot?.querySelector('mud-spinner')?.getAttribute('variant')).toBe('dark');
  });
  ...
  it('focuses the internal control via delegatesFocus', async () => {
    const { root, waitForChanges } = await render(<mud-service-button>Pay</mud-service-button>);
    await waitForChanges();
  ...
  it('href-mode click: handleClick early-returns so native anchor navigation proceeds', async () => {
    const { root, waitForChanges } = await render(
      <mud-service-button href="https://mpay.gov.md">Pay</mud-service-button>,
    );
    await waitForChanges();
  ...
  it('type=submit click: requestSubmit() + setFormValue(name,value) + microtask clears submitter', async () => {
    const { root, waitForChanges } = await render(
      <mud-service-button type="submit" name="action" value="pay">
        Pay
      </mud-service-button>,
    );
    await waitForChanges();
  ...
  it('type=reset click: calls form.reset() via ElementInternals', async () => {
    const { root, waitForChanges } = await render(<mud-service-button type="reset">Reset</mud-service-button>);
    await waitForChanges();
