...
export const AllSizes: Story = {
    ...
    <div style="display: flex; align-items: center; gap: var(--spacing-24); padding: var(--spacing-24); flex-wrap: wrap;">
      ${SIZES.map(
        ...
        <div style="display: flex; flex-direction: column; align-items: center; gap: var(--spacing-8);">
          <mud-spinner size="${size}" variant="brand"></mud-spinner>
  ...
  parameters: {
    ...
    docs: {
      source: {
        code: SIZES.map(size => `<mud-spinner size="${size}" variant="brand"></mud-spinner>`).join('\n'),
      },
    },
  },
};
...
export const AllVariants: Story = {
      ...
      ${VARIANTS.map(
        ...
        <div style="display: flex; flex-direction: column; align-items: center; gap: var(--spacing-8);">
          <div style="${swatchStyle(variant)}">
            <mud-spinner size="md" variant="${variant}"></mud-spinner>
          </div>
  ...
  parameters: {
    ...
    docs: {
      source: {
        code: VARIANTS.map(variant => `<mud-spinner size="md" variant="${variant}"></mud-spinner>`).join('\n'),
      },
    },
  },
};
        ...
        variant => /*html*/ `
          ...
          ${SIZES.map(
            ...
            <div style="${swatchStyle(variant)}; justify-content: center;">
              <mud-spinner size="${size}" variant="${variant}"></mud-spinner>
            </div>`,
          ).join('')}
        `,
      ).join('')}
    ...
    docs: {
      source: {
        code: VARIANTS.map(
          variant =>
            `<!-- variant: ${variant} -->\n` +
            SIZES.map(size => `<mud-spinner size="${size}" variant="${variant}"></mud-spinner>`).join('\n'),
        ).join('\n\n'),
      },
    },
  },
...
export const CoverageGuard: Story = {
  tags: ['!autodocs', '!dev'],
  render: () => /*html*/ `<mud-spinner></mud-spinner>`,
  parameters: {
    controls: { disable: true },
    docs: { disable: true },
  },
  play: async () => {
    const Ctor = customElements.get('mud-spinner') as unknown as (new (registerHost: boolean) => unknown) | undefined;
    if (!Ctor) throw new Error('mud-spinner constructor missing from registry');
...
export const ReducedMotion: Story = {
  render: () => /*html*/ `
    ...
    <div class="reduced-motion-wrapper">
      ${SIZES.map(size => /*html*/ `<mud-spinner size="${size}" variant="brand" label="Loading (reduced motion)"></mud-spinner>`).join('')}
    </div>
...
<div class="reduced-motion-wrapper">
  <mud-spinner size="md" variant="brand" label="Loading (reduced motion)"></mud-spinner>
</div>`,
      },
    },
  },
