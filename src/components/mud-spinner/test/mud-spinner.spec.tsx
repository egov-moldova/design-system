...
describe('mud-spinner', () => {
  ...
  it('exposes the WCAG-required status-role contract', async () => {
    const { root } = await render(<mud-spinner label="Saving" />);
  ...
  it('constructs without registering a host when registerHost=false', () => {
    const Ctor = customElements.get('mud-spinner') as unknown as new (registerHost: boolean) => unknown;
