...
describe('mud-switch', () => {
  describe('defaults + prop reflection', () => {
    ...
    it('renders an internal <input type="checkbox" role="switch">', async () => {
      const { root } = await render(<mud-switch label="x"></mud-switch>);
  ...
  describe('checked state + form association', () => {
    it('reflects checked attribute to host when initially set', async () => {
      const { root } = await render(<mud-switch label="x" checked></mud-switch>);
    ...
    it('emits mudChange with the new checked state when toggled', async () => {
      ...
      const { root } = await render(<mud-switch label="x" value="da" onMudChange={onChange}></mud-switch>);
      const native = queryNative(root)!;
      native.checked = true;
    ...
    it('toggles back from on to off via native change', async () => {
      const onChange = vi.fn();
      const { root } = await render(<mud-switch label="x" checked onMudChange={onChange}></mud-switch>);
      const native = queryNative(root)!;
    ...
    it('does not emit mudChange when toggled while disabled', async () => {
      const onChange = vi.fn();
      const { root } = await render(<mud-switch label="x" disabled onMudChange={onChange}></mud-switch>);
      const native = queryNative(root)!;
      native.checked = true;
      native.dispatchEvent(new Event('change', { bubbles: true }));
      await flush();
    ...
    it('emits mudFocus and mudBlur and toggles the is-focused class', async () => {
      const onFocus = vi.fn();
      const onBlur = vi.fn();
      const { root } = await render(<mud-switch label="x" onMudFocus={onFocus} onMudBlur={onBlur}></mud-switch>);
      const native = queryNative(root)!;
    ...
    it('restores checked state from a non-empty saved form value', async () => {
      const { root } = await render(<mud-switch label="x" value="da"></mud-switch>);
  ...
  describe('Space key toggles via native semantics', () => {
    it('native change (Space-equivalent) emits mudChange + flips is-checked', async () => {
      ...
      // which is the only signal mud-switch listens to.
      const onChange = vi.fn();
      const { root } = await render(<mud-switch label="x" onMudChange={onChange}></mud-switch>);
      const native = queryNative(root)!;
      native.checked = true;
      native.dispatchEvent(new Event('change', { bubbles: true }));
      await flush();
  ...
  describe('label rendering (slot-first)', () => {
    ...
    it('omits the visible label when no slot is provided', async () => {
      const { root } = await render(<mud-switch aria-label="Toggle"></mud-switch>);
      expect(root?.classList.contains('has-label')).toBe(false);
    });
  });
  ...
  describe('disabled behavior', () => {
    it('passes disabled through to the native input', async () => {
      const { root } = await render(<mud-switch label="x" disabled></mud-switch>);
      const native = queryNative(root);
  ...
  describe('ARIA switch contract', () => {
    ...
    it('uses explicit aria-label when no slot is present', async () => {
      const { root } = await render(<mud-switch aria-label="Notificări"></mud-switch>);
      const native = queryNative(root);
    ...
    it('exposes aria-required when required', async () => {
      const { root } = await render(<mud-switch label="x" required></mud-switch>);
