...
@Component({
  tag: 'mud-table',
  styleUrl: 'mud-table.css',
  shadow: true,
})
export class MudTable {
  /**
  ...
  @Event() mudRowClick!: EventEmitter<TableRowClickDetail>;

  /** Emitted when the selection set changes. */
  @Event() mudSelectionChange!: EventEmitter<TableSelectionChangeDetail>;

  @Watch('headerStyle')
  validateHeaderStyle(next: TableHeaderStyle) {
    if (!TABLE_HEADER_STYLES.includes(next)) {
      console.warn(`[mud-table] Invalid headerStyle="${next}". Falling back to "default".`);
  ...
  validateRowStyle(next: TableRowStyle) {
    if (!TABLE_ROW_STYLES.includes(next)) {
      console.warn(`[mud-table] Invalid rowStyle="${next}". Falling back to "divided".`);
  ...
  validateSortDirection(next: TableSortDirection | undefined) {
    ...
    if (!TABLE_SORT_DIRECTIONS.includes(next)) {
      console.warn(`[mud-table] Invalid sortDirection="${next}". Falling back to "asc".`);
  ...
  private handleSort = (column: TableColumn): void => {
    ...
    this.mudSort.emit({ column: column.key, direction: nextDirection });
  };
  ...
  private handleRowClick = (event: MouseEvent, row: TableRowData, index: number): void => {
    ...
    if (target?.closest('[data-table-selection]') || target?.closest('mud-button, mud-checkbox, a, button')) {
      return;
    }
    this.mudRowClick.emit({ row, index });
  };
  ...
  private handleSelectAll = (event: Event): void => {
    ...
    this.mudSelectionChange.emit({ selectedRows: nextSelected });
  };
  ...
  private handleRowSelect = (event: Event, rowId: string): void => {
    ...
    this.mudSelectionChange.emit({ selectedRows: next });
  };
  ...
  private renderSortIcon(column: TableColumn) {
    ...
    return (
      <mud-icon class={{ 'sort-icon': true, 'sort-icon--active': isActive }} name={name} size={16} aria-hidden="true" />
    );
  }
