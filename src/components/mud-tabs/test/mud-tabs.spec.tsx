...
// The JSX typing of `mud-tab` is generated by Stencil into `src/components.d.ts`
// as a `OneOf3` union over its required `value` prop. `yarn typecheck` compiles
// this file, so the expressions below fail the check if the committed union
// drops the `prop:` branch or stops rejecting a doubled binding
// (stenciljs/core#6854). The committed file only proves what was generated last;
// the compiler assertion proves the installed Stencil still generates it, which
// stops holding if the yarn patch in root `resolutions` stops matching.
describe('mud-tab JSX typing', () => {
  it('accepts the required value through the `prop:` prefix', async () => {
    const { root } = await render(<mud-tab prop:value="profil"></mud-tab>);
    expect((root as HostWithTabs).value).toBe('profil');
  });

  it('accepts exactly one of `value`, `attr:value` and `prop:value`', () => {
    void (<mud-tab attr:value="profil"></mud-tab>);
    // @ts-expect-error: `value` and `prop:value` are mutually exclusive
    void (<mud-tab value="profil" prop:value="documente"></mud-tab>);
    // @ts-expect-error: `attr:value` and `prop:value` are mutually exclusive
    void (<mud-tab attr:value="profil" prop:value="documente"></mud-tab>);
  });

  it('is generated by an installed compiler that emits the `prop:` branch', () => {
    const compiler = readFileSync(join(process.cwd(), 'node_modules/@stencil/core/compiler/stencil.js'), 'utf8');
    expect(compiler).toContain('type OneOf3<K extends string, PropT, AttrT = PropT>');
  });
});
