...
// as a `OneOf` union over its required `value` prop. `yarn typecheck` compiles
// this file, so the `@ts-expect-error` below fails the check if the generated
// union stops rejecting a doubled binding, and the plain `prop:value` render
// fails it if the union drops the `prop:` branch (stenciljs/core#6854).
describe('mud-tab JSX typing', () => {
  it('accepts the required value through the `prop:` prefix', async () => {
    ...
    expect((root as HTMLElement & { value?: string }).value).toBe('profil');
  });

  it('rejects the required value bound twice', async () => {
    // @ts-expect-error: `value` and `prop:value` are mutually exclusive
    const { root } = await render(<mud-tab value="profil" prop:value="documente"></mud-tab>);
