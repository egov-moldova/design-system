...
| `clearLabel`   | `clear-label`  | Accessible label for the clear (×) button. Only used when `clearable` is set.                                                                                                                                                                                                       | `string`                                                        | `'Golește câmpul'` |
| `clearable`    | `clearable`    | Shows a trailing clear (×) button while the control holds a value. Clearing empties the field, emits `mudInput` + `mudChange`, and returns focus to the input. Suppressed when disabled, read-only, or loading.                                                                     | `boolean`                                                       | `false`            |
