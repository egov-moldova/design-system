...
describe('mud-textarea', () => {
  describe('defaults + prop reflection', () => {
    it('renders with default props reflected on host', async () => {
      const { root } = await render(<mud-textarea label="Comentarii"></mud-textarea>);
  ...
  describe('shadow structure', () => {
    ...
    it('adds a required mark when `required` is set', async () => {
      const { root } = await render(<mud-textarea label="x" required></mud-textarea>);
    ...
    it('renders a helper assistive row when `helper-text` is set', async () => {
      const { root } = await render(<mud-textarea label="x" helper-text="Helpful tip"></mud-textarea>);
    ...
    it('renders an error assistive row with the error icon when invalid + error-text', async () => {
      const { root } = await render(<mud-textarea label="x" invalid error-text="Required"></mud-textarea>);
      const assistive = queryAssistive(root);
    ...
    it('error message takes priority over helper text', async () => {
      const { root } = await render(
        <mud-textarea label="x" invalid helper-text="Hint" error-text="Required"></mud-textarea>,
      );
      const assistive = queryAssistive(root);
      expect(assistive?.textContent).toContain('Required');
    ...
    it('hides the resize grip when resize="none"', async () => {
      const { root } = await render(<mud-textarea label="x" resize="none"></mud-textarea>);
      const grip = queryResizeGrip(root);
  ...
  describe('character counter', () => {
    ...
    it('renders the counter when maxLength is set', async () => {
      const { root } = await render(<mud-textarea label="x" maxLength={50}></mud-textarea>);
    ...
    it('updates the counter as value grows', async () => {
      const { root } = await render(<mud-textarea label="x" value="abc" maxLength={50}></mud-textarea>);
      const counter = queryCounter(root);
      expect(counter?.textContent?.replace(/\s+/g, ' ').trim()).toBe('3 / 50');
    });

    it('adds counter-over class when value exceeds the limit', async () => {
      const { root } = await render(<mud-textarea label="x" value="abcdef" maxLength={3}></mud-textarea>);
      expect(root?.classList.contains('counter-over')).toBe(true);
    });

    it('suppresses the counter when show-counter is false', async () => {
      const { root } = await render(<mud-textarea label="x" maxLength={50} showCounter={false}></mud-textarea>);
      expect(queryCounter(root)).toBeNull();
    });
  ...
  describe('value + form association', () => {
    it('renders value inside the native textarea', async () => {
      const { root } = await render(<mud-textarea label="x" value="hello"></mud-textarea>);
    ...
    it('emits mudInput on each keystroke', async () => {
      const onInput = vi.fn();
      const { root } = await render(<mud-textarea label="x" onMudInput={onInput}></mud-textarea>);
      const native = queryNative(root)!;
      native.value = 'a';
    ...
    it('emits mudChange on change (blur)', async () => {
      ...
      const { root } = await render(<mud-textarea label="x" onMudChange={onChange}></mud-textarea>);
      const native = queryNative(root)!;
      native.value = 'done';
    ...
    it('emits mudFocus and mudBlur and toggles the is-focused class', async () => {
      const onFocus = vi.fn();
      const onBlur = vi.fn();
      const { root } = await render(<mud-textarea label="x" onMudFocus={onFocus} onMudBlur={onBlur}></mud-textarea>);
      const native = queryNative(root)!;
  ...
  describe('disabled + readonly behavior', () => {
    it('passes disabled through to the native textarea', async () => {
      const { root } = await render(<mud-textarea label="x" disabled></mud-textarea>);
      const native = queryNative(root);
    ...
    it('passes readonly through to the native textarea', async () => {
      const { root } = await render(<mud-textarea label="x" readonly value="x"></mud-textarea>);
      const native = queryNative(root);
  ...
  describe('ARIA contract', () => {
    ...
    it('exposes aria-invalid when invalid', async () => {
      const { root } = await render(<mud-textarea label="x" invalid></mud-textarea>);
    ...
    it('wires aria-describedby to the helper id when helper-text present', async () => {
      const { root } = await render(<mud-textarea label="x" helper-text="hint"></mud-textarea>);
    ...
    it('uses aria-label as the accessible name when no visible label is present', async () => {
      const { root } = await render(<mud-textarea aria-label="Comentarii"></mud-textarea>);
      const native = queryNative(root);
  ...
  describe('variant matrix', () => {
    it.each(['default', 'warning', 'destructive', 'success'] as const)(
      ...
      async variant => {
        const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
        const { root } = await render(<mud-textarea label="x" variant={variant}></mud-textarea>);
        expect(root?.getAttribute('variant')).toBe(variant);
  ...
  describe('form validity', () => {
    ...
    it('sets valueMissing on transition from filled to empty when required', async () => {
      const { root } = await render(<mud-textarea label="x" required value="hello"></mud-textarea>);
