...
| -------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `content`      | `content`       | Convenience: tooltip body text. Rendered as the default slot's fallback, so only when the host has no default-slot nodes at all — whitespace between tags counts as a node.                                                                                                                                                         | `string \| undefined`                                                                                                                                                              | `undefined` |
...
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------- |
| `mudClose` | Fired when the tooltip is hidden. `detail.reason` records the cause: `'blur'` (mouseleave / focusout), `'escape'` (Esc), `'close-button'` (coach variant close), `'click-outside'` (outside click), or `'click-trigger'` (second click on the trigger when `trigger="click"`). | `CustomEvent<TooltipCloseEventDetail>` |
| `mudOpen`  | Fired when the tooltip becomes visible (after `showDelay` for hover triggers).                                                                                                                                                                                                 | `CustomEvent<void>`                    |
...
```mermaid
graph TD;
  mud-breadcrumb --> mud-tooltip
  mud-breadcrumb-item --> mud-tooltip
  style mud-tooltip fill:#f9f,stroke:#333,stroke-width:4px
```
